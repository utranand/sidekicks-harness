...
//
// The PowerShell four were added 2026-09-05: the list had been written from this repo's own
// POSIX corpus, so a skill authored on Windows could ship its entire executable surface in `.ps1`
// and the scanner would see an empty folder — `manifestRequired()` would then report that no
// manifest was needed and `skill doctor` would stay green over an undeclared closure
// (INC-2026-09-05-02, X-5).
const SCANNED_EXT = /\.(md|mjs|js|cjs|ts|sh|bash|ps1|psm1|cmd|bat|py|ya?ml|json|toml|txt)$/i;
...
const EXTERNAL_BINARIES = Object.freeze([
  // `pwsh` earns its place the same way the rest do: a `.ps1` under scripts/ does not run on
  // macOS or Linux without it, and PowerShell is NOT preinstalled on either — the exact "present
  // on any developer machine" test that keeps curl and git off this list, failed.
  'pwsh',
...
export function stripComments(text, file) {
  ...
  // PowerShell: `<# … #>` block comments, then `#` to end of line — the same shape as sh, so the
  // two share the line rule and PowerShell adds the block form.
  if (/\.psm?1$/i.test(file)) {
    return text
      .replace(/<#[\s\S]*?#>/g, blank)
      .replace(/(^|[ \t\r\n])#[^\n]*/g, blank);
  }
  // Batch: REM lines and `::` labels-as-comments.
 ...
 *
 * A SYMLINK IS NEVER FOLLOWED, AND NEVER A FILE ROW. This walk used to `statSync` every entry,
 * which resolves links: a link to a directory was recursed into and a link to a file was read as
 * its target, so bytes that live OUTSIDE the skill folder were copied INTO it by every consumer of
 * this list — import's apply loop, the export bundle, the receipt's `files:` hashes. That broke the
 * two promises the copy rests on at once (byte-exact, and a liftable folder), and it broke them
 * ASYMMETRICALLY: git on Windows defaults to `core.symlinks=false` and writes a link as a small
 * text file holding its target path, so the same upstream commit presented this walk with a
 * different file set per platform and produced different receipt hashes on two machines with no
 * warning on either (INC-2026-09-05-02, X-1).
 *
 * `readdirSync(…, {withFileTypes: true})` already answers `isSymbolicLink()` from the lstat it had
 * to do anyway, so declining to follow costs nothing. Links are reported through `opts.onSymlink`
 * rather than returned, so no existing caller has to change to stay correct — a caller that does
 * not care simply never hears about them, and a caller that must decide (adoptionPlan) does.
 *
 * @param {string} skillDir
 * @param {{onSymlink?: (link: {rel: string, abs: string, target: string|null, dangling: boolean, inside: boolean}) => void}} [opts]
 * @returns {Array<{rel: string, abs: string, inScripts: boolean}>}
 */
export function walkSkillFiles(skillDir, opts = {}) {
  const out = [];
  const onSymlink = typeof opts.onSymlink === 'function' ? opts.onSymlink : null;
  const walk = (dir) => {
    let entries;
    try {
    ...
    for (const e of entries) {
      ...
      if (e.isSymbolicLink()) {
        if (onSymlink) onSymlink(describeLink(skillDir, abs));
        continue;                       // never followed, never a file row
      }
      let st;
      try {
        st = statSync(abs);
      } catch {
        continue;                       // an unreadable entry is not a file this skill owns
      }
      if (st.isDirectory()) {
        walk(abs);
        continue;
...
/**
 * What one symlink inside a skill folder is, for the caller that has to decide about it.
 *
 * `inside` is the load-bearing field: a link whose target resolves within the skill folder can be
 * materialised as a byte copy and stay deterministic on every OS, while one pointing outside means
 * the folder is not liftable and no copy of it can be honest.
 *
 * @param {string} skillDir
 * @param {string} abs
 * @returns {{rel: string, abs: string, target: string|null, dangling: boolean, inside: boolean}}
 */
function describeLink(skillDir, abs) {
  let target = null;
  try { target = readlinkSync(abs); } catch { target = null; }
  const resolved = target === null ? null : resolve(dirname(abs), target);
  const root = resolve(skillDir);
  const inside = resolved !== null && (resolved === root || resolved.startsWith(root + sep));
  return {
    rel: posix(relative(skillDir, abs)),
    abs,
    target: target === null ? null : posix(target),
    dangling: resolved === null || !existsSync(resolved),
    inside,
  };
}
...
export function scanSkill(repoRoot, entry, universe, ctx = {}) {
  ...
  return {
    skill: self,
    // The folder itself, so a consumer that must ask the SOURCE about its files (the git index,
    // for the executability baseline) does not have to reconstruct it from a file's abs path.
    dir: entry.dir,
    files,
 ...
 * @param {{runtimeClass?: string|null}} [opts] - `runtimeClass` is the skill's declared
 *   `sidekicks.runtime-class` (lib/skill-manifest/read.mjs readSkillRuntimeClass)
 * @returns {{required: boolean, because: string[]}}
 */
export function manifestRequired(scan, hasDescriptor, read = null, opts = {}) {
  const because = [];
  // A skill that declares itself part of the framework RUNTIME ships in every forged core, and
  // framework publication projects a runtime subset of each skill folder. That projection needs a
  // complete hash baseline to describe what it carried: without `bundle{}` the only fallback is a
  // directory walk, which cannot distinguish "this file was excluded on purpose" from "this file is
  // missing". So the declaration itself is the reason — derived per skill, not a publisher list, and
  // visible to `skill doctor` and `skill manifest --check` rather than only at forge time.
  if (opts.runtimeClass === 'framework') {
    because.push('declares runtime-class: framework — a shipped skill needs a hash baseline');
  }
