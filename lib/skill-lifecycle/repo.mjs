...
// `sidekicks skill repo init <path> [--private] [--name <n>] [--remote <url>] [--peer <public-repository-name>] [--json]`
//
...
export async function run(ctx, args) {
  ...
  if (!SUBVERBS.includes(subverb)) {
    throw new SidekicksError(
      ...
      + '  sidekicks skill repo init <path> [--private] [--name <n>] [--remote <url>] '
...
async function init(ctx) {
  ...
  // Keep the source checkout's conventional peer relationship without baking a private
  // repository name into the framework runtime. An operator can always make the relationship
  // explicit with --peer; the default is derived solely from the repository name they supplied.
  if (isPrivate && !flags.peer && !name.endsWith('-private')) {
    throw new SidekicksError(
      `skill repo init: private repository name '${name}' has no unambiguous public peer; `
      + 'pass --peer <public-repository-name>',
      EXIT_VALIDATION
    );
  }
  const peer = flags.peer
    ? String(flags.peer)
    : (isPrivate ? name.slice(0, -'-private'.length) : `${name}-private`);
  const vars = {
    REPO_NAME: name,
    PEER_REPO: peer,
  ...
  const planned = [
    ...
    // and from layout 2 on the skill folders no longer create one — so without this every sidekicks
    // verb run inside the clone walks up out of it. Export writes the same placeholder; both are
    // needed, because a repo is scaffolded before it is ever exported to. The `.sidekicks/` DIRECTORY
    // is the whole requirement, so the marker sits directly in it — layout 2 put it under
    // `skill-offloaded/`, which conjured a parked tree in a repo that publishes no parked skills.
    { path: '.sidekicks/.gitkeep', content: '' },
...
function render(r) {
  ...
  const published = [...new Set(Object.values(PUBLISHED_TREES))].join(', ');
  out.push(
    '',
