 ...
 *
 * `config sync --scope all --prune-only` is invoked as a FUNCTION, not re-implemented: it walks live
 ...
 *
 * PRUNE-ONLY, not `--prune`. A plain `--prune` run is a full sync with pruning added, and a full
 * sync also SEEDS every declared block that is absent from a scope — so removing a skill that owned
 * no config block at all wrote six unrelated family files into two other projects. Reusing a broad
 * verb in process inherits its side effects; the narrow mode is the fix.
 *
 ...
 */
/**
 * The receipt's `source:` block as one sentence.
 *
 * It is a MAPPING (profile.mjs renders kind/destination/remote/commit/branch), so interpolating it
 * into the report printed `imported    [object Object]` — and the same object went out in --json,
 * where a consumer expecting a string got one too.
 *
...
function describeSource(source) {
  ...
  const where = say(source.destination) || say(source.remote);
  const kind = say(source.kind);
  const commit = say(source.commit);
  const bits = [];
  if (kind) bits.push(`a ${kind} source`);
  if (where) bits.push(`at ${where}`);
  if (commit) bits.push(`commit ${commit.slice(0, 12)}`);
...
async function pruneDeclarations(repoRoot) {
  try {
    const result = await configSyncRun(
      { repoRoot, argv: ['config', 'sync', '--scope', 'all', '--prune-only', '--json'], flags: {} },
      {}
    );
...
async function runLocal(ctx, entry, opts) {
  ...
  if (opts.apply) {
    ...
    // `removeProfile` returns path:null under {tombstone:false}, so `profile_retired` alone cannot
    // tell "purged a real receipt" from "there was none" — and the report used to call the first
    // one the second.
    report.profile_purged = Boolean(retired.had && opts.purgeProfile);
    ...
    if (!prune.ok) {
      ...
      report.manual.unshift(
        `'sidekicks config sync --scope all --prune-only' failed in process (${prune.error}) — `
...
function renderLocal(r) {
  ...
  // `imported_from` is the receipt's source MAPPING — --json keeps it structured (consumers read
  // .remote); the human line needs a sentence, or it prints `[object Object]`.
  if (r.imported_from) out.push(`  imported    ${describeSource(r.imported_from)}`);
  ...
  if (!r.dry_run) {
    ...
    out.push(`  receipt           ${
      r.profile_retired ? `retired to ${r.profile_retired}`
        : r.profile_purged ? 'purged (--purge-profile — no tombstone kept)'
          : '(none to retire)'}`);
    if (r.emptied.length) {
