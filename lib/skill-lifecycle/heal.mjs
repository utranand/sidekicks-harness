...
import { modeFromDigits } from '../skill-manifest/mode.mjs';
 ...
 */
/**
 * How to create the single repo-root `.venv`, on the platform asking.
 *
 * `python3` is not a command on a default Windows install — the launcher is `py`, and a bare
 * `python` is whatever happens to be on PATH (often the Microsoft Store stub, which is why `py -3`
 * leads). Same two-string shape `install_hint` / `install_hint_windows` already uses for binaries;
 * the venv line was the one piece of remediation still hard-coded to one platform, inside a module
 * whose own pip lookup right below has been platform-aware all along (INC-2026-09-05-02, X-7).
 *
 * @param {string} [platform]
 * @returns {string}
 */
export function venvCreateHint(platform = process.platform) {
  return platform === 'win32'
    ? "py -3 -m venv .venv (or 'python -m venv .venv' where the py launcher is absent)"
    : 'python3 -m venv .venv';
}
...
export function healPlan(repoRoot, entry, opts = {}) {
  ...
  for (const [rel, recorded] of Object.entries(m.bundle || {})) {
    ...
    actions.push({
      ...
      // The manifest records executability separately from content, so restoring the bytes without
      // restoring the bit would put back a script that cannot run. Absent `modes{}` means the
      // baseline predates mode recording — 644 is then the honest default, not a claim.
      mode: modeFromDigits((m.modes || {})[rel] === 755 ? 755 : 644),
  ...
  if (wanted.length && opts.pip !== false) {
    ...
    if (!pip) {
      residue.push({
        ...
        remediation: `create it once with '${venvCreateHint()}' — heal never creates it, because `
    ...
    for (const row of rows) {
      ...
      residue.push({
        ...
        remediation: section === 'framework_hooks'
          ...
          : (section === 'binaries'
            // The Windows hint FALLS BACK to the generic one. It used to short-circuit straight
            // to 'install it on the host' whenever a row declared only `install_hint`, so a
            // Windows operator got strictly LESS help than a macOS one out of the same manifest
            // — the reverse of what the two-string pattern exists for.
            ? (process.platform === 'win32'
              ? (row.install_hint_windows || row.install_hint)
              : row.install_hint)
...
export function applyHeal(repoRoot, actions) {
  ...
  for (const a of actions.filter((x) => x.verb === 'restore-file')) {
    try {
      ...
      writeAtomic(abs, a._content, { mode: a.mode });
