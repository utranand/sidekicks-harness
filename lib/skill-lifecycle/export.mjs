...
//
// LAYOUT. A destination receives `<output>/.agents/skills/<name>` for EVERY skill, parked here or
// not (layout 3, LAYOUT_VERSION in lib/skill-package/portable.mjs) — a destination never carries a
// `skill-offloaded/` tree, because being parked is a fact about this repo's discovery and not about
// the published copy. The destination path is still looked up through `publishedTreeFor()` rather
// than derived from a tree basename: the local trees do not share a parent, and re-joining a basename
// onto a fixed prefix writes to a directory nothing reads while reporting success.
//
...
import {
  configuredDestinations, readDestinationIntent, exportSettings, publishedTreeFor, DEST_TREES,
...
export function readPreset(repoRoot, name) {
  ...
  if (name === 'framework' && foundTarget) {
    const requiredFloor = readPreset(repoRoot, 'required') || [];
    const resolved = resolveFrameworkPreset(repoRoot, { requiredFloor });
    if (resolved.errors.length) {
      throw new SidekicksError(
        `skill export: framework preset cannot be composed:\n  ${resolved.errors.join('\n  ')}`,
        EXIT_VALIDATION
      );
    }
...
/**
 * This export's rows, plus the destination's existing rows for skills it did not touch.
 *
 * A parsed catalog row is FLAT (`name`, `siblings`, `bundle_files`) while renderCatalogRow wants the
 * facts shape (`skill`, `outside_edges.sibling_skills`, `file_count`), so a carried row is mapped
 * back rather than re-emitted verbatim — which also means a row this reader cannot understand is
 * dropped loudly by the refusal below rather than written out half-formed.
 *
 * @param {string} outputBase - the destination root
 * @param {object[]} fresh - facts rows for the skills THIS export wrote
 * @returns {{rows: object[], carried: string[]}}
 */
function mergeCatalogRows(outputBase, fresh) {
  const abs = join(outputBase, 'catalog.yaml');
  if (!existsSync(abs)) return { rows: fresh, carried: [] };

  let existing;
  try {
    existing = parse(readFileSync(abs, 'utf8'));
  } catch (err) {
    throw new SidekicksError(
      `skill export: refusing to rewrite an unreadable catalog.yaml at ${abs} (${err.message}). `
      + 'Regenerating it from this export alone would drop every row it holds — fix or remove the '
      + 'file, or re-export the whole repository.',
      EXIT_VALIDATION
    );
  }
  if (!existing || !Array.isArray(existing.skills)) {
    // A catalogue with no readable `skills:` list is either a genuinely empty destination or a
    // damaged file. Only the folders on disk tell the two apart, and getting it wrong the generous
    // way is what this whole function exists to prevent: regenerating from this export alone would
    // drop every row the file was supposed to hold.
    const populated = DEST_TREES.some((t) => {
      try {
        return readdirSync(join(outputBase, ...t.split('/')), { withFileTypes: true })
          .some((e) => e.isDirectory());
      } catch { return false; }
    });
    if (!populated) return { rows: fresh, carried: [] };
    throw new SidekicksError(
      `skill export: refusing to rewrite ${abs} — it carries no readable 'skills:' list while the `
      + 'destination holds published skills, so regenerating it from this export alone would drop '
      + 'every row it was meant to hold. Fix or remove the file, or re-export the whole repository.',
      EXIT_VALIDATION
    );
  }

  const written = new Set(fresh.map((r) => r.skill));
  const rows = [...fresh];
  const carried = [];
  for (const row of existing.skills) {
    const name = row && row.name;
    if (!name || written.has(name)) continue;
    // Only a skill still ON DISK at the destination. A row whose folder is gone was retracted.
    if (!DEST_TREES.some((t) => existsSync(join(outputBase, ...t.split('/'), name)))) continue;
    rows.push({
      skill: name,
      version: row.version || '',
      group: row.group || '',
      category: row.category || row.group || '',
      first_party: row.first_party === true || String(row.first_party) === 'true',
      manifest: row.manifest === true || String(row.manifest) === 'true',
      python: String(row.python || '').split(',').map((x) => x.trim()).filter(Boolean),
      config_block: row.config_block || '',
      file_count: Number(row.bundle_files || 0),
      outside_edges: {
        sibling_skills: String(row.siblings || '').split(',').map((x) => x.trim()).filter(Boolean),
        binaries: String(row.binaries || '').split(',').map((x) => x.trim()).filter(Boolean),
      },
    });
    carried.push(name);
  }
...
export async function run(ctx, args) {
  ...
  const report = {
    ...
    reference_copies: [],
    // Destination rows this export did not touch and preserved. Reported so a partial export says
    // out loud that it kept the rest of the index rather than silently rewriting it.
    catalog_carried: [],
  };
  ...
  if (!dryRun) {
    ...
    // directory always existed. Layout 2 onwards publishes skills to `.agents/skills/`, so a
    // destination would have no `.sidekicks/` at all and every verb run there would die with "not
    // inside a Sidekicks repository" — walking up out of the destination and into whatever repo
    // happens to contain it, which is worse than failing.
    //
    // So the placeholder is written explicitly, and at `.sidekicks/.gitkeep` rather than inside a
    // `skill-offloaded/` subdirectory: `resolveRepoRoot` needs the `.sidekicks/` DIRECTORY and nothing
    // else, and layout 2's placeholder path conjured a parked tree at every destination that no
    // published skill has ever lived in.
    writeAtomic(join(outputBase, '.sidekicks', '.gitkeep'), '');
    for (const u of plan.units) {
      // The WHOLE tree path is mapped, never a basename re-joined onto a fixed prefix. The local
      // `.agents/skills` and `.sidekicks/skill-offloaded` do not share a parent, so a basename cannot
      // say where a skill publishes — and the failure mode of pretending it can is a write to a
      // directory nothing reads, reported as success. Both map to `.agents/skills` at the destination
      // under layout 3; `tree:` in origin.yaml carries that full path for the same reason.
    ...
    // MERGE, DO NOT CLOBBER. renderCatalog builds the file from the rows it is handed, so passing
    // only this export's units rewrote the destination's whole index — a three-skill export into a
    // populated repository took its catalogue from 100 rows to 3, which is exactly what the public
    // checkout is carrying uncommitted right now. Every README generator and `skill destinations`
    // then reads a near-empty repository.
    //
    // Carried-over rows are the destination's own, for skills this export did not touch and that
    // are still present there. A skill retracted from the destination drops out because its folder
    // is gone; export never deletes, so nothing else can.
    const fresh = plan.units.map((u) => ({
    ...
    const { rows: catalogRows, carried } = mergeCatalogRows(outputBase, fresh);
    writeAtomic(
    ...
    report.catalog_carried = carried;
  ...
  out.push('');
  if (report.catalog_carried.length) {
    out.push(`catalog.yaml: ${report.catalog_carried.length} existing row(s) preserved `
      + '(this export did not touch them)');
    out.push('');
  }
