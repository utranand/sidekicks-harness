...
import { readSource } from '../skill-package/source-layout.mjs';
import { scanSkill, walkSkillFiles } from './scan.mjs';
...
/**
 * Advise on a skill that is NOT installed here yet, read from the source folder.
 *
 * Deliberately a REDUCED answer, and the row says so. `advisePlan` leans on `skillClosure` and
 * `auditSkills`, both of which walk the LOCAL tree; a foreign folder has no place in either. What
 * the scanner can still see from the folder alone — the sibling skills it reaches for, and whether
 * this repo has them — is the part that decides whether an import is worth starting.
 */
function foreignRow(repoRoot, name, entry, catalog) {
  const present = new Set(discoverSkills(repoRoot).map((e) => e.skill));
  let scan = { skills: [], binaries: [], frameworkFiles: [] };
  try {
    const files = walkSkillFiles(entry.dir);
    scan = scanSkill(entry.dir, { skill: name, dir: entry.dir, relDir: entry.relDir || '' }, [], { files });
  } catch { /* an unreadable foreign folder advises nothing, it does not fail the run */ }
  return {
    skill: name,
    group: null,
    not_installed_here: true,
    source_path: entry.upstreamRel || entry.relDir || '',
    required: (scan.skills || []).map((sib) => ({
      skill: sib.skill,
      how: sib.how || 'detected',
      scope: 'runtime',
      degraded: sib.degraded || '',
      present: present.has(sib.skill),
      offloaded: false,
      available_in_catalog: catalog ? catalog.has(sib.skill) : null,
    })),
    required_transitive: [],
    mirror_only: readFrontmatterDependsOn(entry).filter((d) => !present.has(d)),
    consider: [],
    non_healable: {
      framework_files: (scan.frameworkFiles || []).map((f) => f.path),
      framework_hooks: [],
      host_paths: [],
      binaries: (scan.binaries || []).map((b) => b.name),
    },
    needs_bmad: /^sk-bmad-/.test(name),
  };
}
...
export async function run(ctx) {
  ...
  // `--from` IS READ FIRST, before the name check. It used to be read after, so a skill that exists
  // only in the source was rejected outright — and "what will this need if I import it?" is exactly
  // the question worth asking BEFORE an import, which is the one moment the skill is not local yet.
  let catalog = null;
  ...
  if (typeof flags.from === 'string' && flags.from) {
    ...
    try { source = readSource(fromRoot); } catch { source = null; }
  }

  const known = new Set(discoverSkills(ctx.repoRoot).map((s) => s.skill));
  // A name that is not here but IS in the source gets advised FROM the source.
  const foreign = new Map();
  for (const n of names) {
    if (known.has(n)) continue;
    const entry = source && source.entries ? source.entries.get(n) : null;
    if (entry) foreign.set(n, entry);
  }
  const unknown = names.filter((n) => !known.has(n) && !foreign.has(n));
  if (unknown.length) {
    throw new SidekicksError(
      `skill advise: unknown skill(s): ${unknown.join(', ')} — run 'sidekicks skill list'`
      + (flags.from ? `, or 'sidekicks skill import --list --from ${flags.from}'` : ''),
      EXIT_VALIDATION
    );
  }

  const rows = advisePlan(ctx.repoRoot, names.filter((n) => !foreign.has(n)), { catalog });
  for (const [name, entry] of foreign) rows.push(foreignRow(ctx.repoRoot, name, entry, catalog));
  ...
  for (const r of rows) {
    ...
    if (r.not_installed_here) {
      // Say the answer is reduced. skillClosure and auditSkills both walk the LOCAL tree, so a
      // foreign folder gets what the scanner alone can see — which is the part that decides whether
      // the import is worth starting, and not the same thing as a full advise.
      out.push(`  NOT INSTALLED HERE — read from the source at ${r.source_path || '(source)'}.`);
      out.push('  Declared/detected siblings only; the transitive closure and audit findings need');
      out.push('  the skill to be present, so import it first for the full answer.');
    }
