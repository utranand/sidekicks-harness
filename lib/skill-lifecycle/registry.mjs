...
import { UNKNOWN, appendHistory } from '../skill-registry/profile.mjs';
...
export async function run(ctx) {
  const flags = parseSkillFlags(
    ctx.argv, ['json', 'check', 'backfill', 'assume-imported', 'apply', 'all']
  );
  ...
  // ── The re-record writer ────────────────────────────────────────────────────────────────────
  //
  // `--check` told operators to "re-record it" and no verb did. Worse, the documented post-import
  // plan CAUSES the drift it complains about: `skill manifest <name> --apply` creates a manifest,
  // which moves `manifest_present` and `file_count`, so following the instructions turned a
  // CI-shaped check red — which is how a check trains people to ignore it.
  //
  // Only `mirror` is ever rewritten. `files{}` is the as-installed baseline that `changedSince()`
  // uses to report `local-only` and that `skill remove` uses to tell a file the import brought from
  // one added since; refreshing it would erase both signals and lie to the removal.
  //
  // `--all` IS allowed here, unlike `--backfill`. The distinction is the whole argument: a backfill
  // ASSERTS where a skill came from, and asserting that wholesale is how a record stops meaning
  // anything. Re-recording a mirror asserts nothing — it copies disk into the half of the file whose
  // documented contract is already "the filesystem wins".
  const rerecorded = [];
  if (flags.apply) {
    if (!names.length && !flags.all) {
      throw new SidekicksError(
        'skill registry --apply: name the skill(s) to re-record, or pass --all. Rewriting every '
        + 'mirror in the repo is a wide write, and it should not be what a bare --apply does.',
        EXIT_USAGE
      );
    }
    const installed = new Map(discoverSkills(ctx.repoRoot).map((e) => [e.skill, e]));
    for (const r of rows) {
      if (r.status !== 'tree-drift') continue;      // a no-op write is churn, not a fix
      const entry = installed.get(r.skill);
      const profile = entry ? readProfile(ctx.repoRoot, r.skill) : null;
      if (!profile) continue;
      const moved = (r.fields || []).join(', ') || 'mirror';
      recordProfile(ctx.repoRoot, {
        ...profile,
        mirror: mirrorFacts(ctx.repoRoot, entry),
        history: appendHistory(profile, {
          at: nowBangkok(), action: 're-record', detail: `mirror refreshed: ${moved}`,
        }),
      });
      rerecorded.push({ skill: r.skill, fields: r.fields || [] });
    }
  }
  // A named skill with no receipt is not an error and is never written: most skills here were
  // authored in this repo. Say what would record one instead.
  const noReceipt = flags.apply
    ? rows.filter((r) => r.status === 'untracked' && names.includes(r.skill)).map((r) => r.skill)
    : [];

  const failing = rows
    .filter((r) => CHECK_FAILS.has(r.status))
    .filter((r) => !rerecorded.some((w) => w.skill === r.skill));
  ...
  if (rerecorded.length) {
    out.push('', `Re-recorded ${rerecorded.length} mirror(s) — files{} untouched:`);
    for (const w of rerecorded) out.push(`  ${w.skill}  (${w.fields.join(', ') || 'mirror'})`);
  }
  for (const n of noReceipt) {
    out.push('', `${n} has no receipt, so there is no mirror to re-record — it was authored here, `
      + `or it predates the registry. Record one with 'skill registry --backfill --assume-imported `
      + `${n} --apply' only if it was in fact imported.`);
  }
