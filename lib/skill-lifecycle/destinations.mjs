...
/**
 * Where a skill from each LOCAL tree is PUBLISHED inside a destination repository — layout 3.
 *
 * BOTH LOCAL TREES PUBLISH TO `.agents/skills`, and that collapse is the point of layout 3. Being
 * parked is a fact about THIS repo's skill discovery — `.sidekicks/skill-offloaded/` exists so a
 * host CLI stops loading a skill here — and it says nothing about the published copy. A destination
 * is a library, not a mirror of one consumer's load state, so a skill lands in the one place every
 * reader (the browse pages, `skill import`, another repo's `--from`) already looks, and a
 * destination never carries a `skill-offloaded/` tree at all. Layout 2 mapped parked→parked, which
 * published a skill into a directory whose whole purpose is to be ignored.
 *
 * STILL A MAP, not a constant and not a prefix swap. Layout 1 published both trees under one parent,
 * so export could take a local tree's basename and re-join it onto a literal `'.sidekicks'`. The
 * local trees have not shared a parent since Rule 3, so a basename still cannot determine a
 * destination path, and re-joining one is the bug shape that already bit
 * `lib/skill-package/source-layout.mjs` (see probe()'s comment) and `import.mjs applyRenames`: it
 * resolves to a directory that does not exist and reads EMPTY rather than failing. Keeping the map
 * also keeps `publishedTreeFor()`'s throw, so a NEW local tree has to declare where it publishes
 * rather than inheriting an answer.
 *
 * Keys are this repo's trees; values are the destination's. A destination is a separate repository on
 * its own release cadence, so the two sides are allowed to diverge and this map is where that is
 * expressed.
 */
export const PUBLISHED_TREES = Object.freeze({
  '.agents/skills': '.agents/skills',
  '.sidekicks/skill-offloaded': '.agents/skills',
});

/**
 * Trees earlier layouts published to, READ-ONLY. Kept forever: a clone published before a bump must
 * still resolve, both for `destinations` status and for `skill remove --destination`'s retraction.
 *
 * `.sidekicks/skill-offloaded` is here because layout 2 published parked skills there; nothing writes
 * it any more. `.sidekicks/skills` is layout 1, which published both trees there.
 */
const LEGACY_DEST_TREES = Object.freeze(['.sidekicks/skill-offloaded', '.sidekicks/skills']);
 ...
 *
 * So the list carries EVERY layout and every use is a first-hit-wins probe: a destination still on
 * layout 1 resolves, one written at layout 2 or 3 resolves, and no caller needs to know which.
 *
 * READ ORDER AGREES WITH WRITE ORDER BY CONSTRUCTION. It is built from `PUBLISHED_TREES` — what
 * `skill export` actually writes — with the superseded trees appended as read tolerance behind them.
...
export async function run(ctx, args) {
  ...
  // The LOCALLY re-parsed positional wins over the dispatcher's. cli.mjs builds `args.name` from a
  // parseArgs that runs strict:false with no option config, so it cannot know `--destination` takes
  // a value and hands the value on as the first positional — `--destination public` then read as
  // "no skill named 'public'". remove.mjs:840 already has this precedence; this verb did not.
  // `args.name` is deliberately NOT a fallback: positionalArgs is a strict superset of the
  // dispatcher's extraction for this verb, so an empty result means there genuinely was no skill
  // name — and `skill destinations --destination public` is exactly that case.
  const skill = names[0] || null;
  ...
  // `unreachable` is counted above and belongs in the tally: without it a run against a checkout
  // that is not on disk ends "in-sync 0 stale 0 retracted 0 never-exported 0", which reads as
  // "nothing to do" when the truth is "nothing could be compared".
  out.push('', `in-sync ${counts['in-sync']}  stale ${counts.stale}  retracted ${counts.retracted}`
    + `  never-exported ${counts['never-exported']}  unreachable ${counts.unreachable}`);
  return { stdout: out.join('\n') + '\n', exitCode: EXIT_OK };
}
