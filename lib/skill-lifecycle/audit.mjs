...
import { frameworkRoots, resolveOwned } from '../sk-cli/core-mount.mjs';
...
export function auditSkills(repoRoot, opts = {}) {
  ...
  // Hook bodies are framework-owned, so in a consumer workspace they live under the mounted core,
  // not necessarily under repoRoot. Resolve this once for the whole audit: checking repoRoot alone
  // made a healthy mounted core fail `skill verify` after its forge had already mutated the target.
  const ownedRoots = frameworkRoots(repoRoot);
  ...
  for (const entry of all) {
    ...
    const { required, because } = manifestRequired(scan, Boolean(descriptor), read, {
      runtimeClass: safeRuntimeClass(entry),
    });
 ...
 */
function auditOne(
  repoRoot, entry, scan, descriptor, read, derived, required, because, add, entryByName, ownedRoots
) {
  ...
  if (decl) {
    ...
    for (const h of decl.framework_hooks) {
      if (!resolveOwned(ownedRoots, h.script.split('/'))) {
        add(
          skill, 'declared-but-absent', 'error',
    ...
    for (const s of decl.sibling_skills) {
      ...
      // An ABSENT sibling is a different finding, already reported above as declared-but-absent /
      // declared-optional-absent, and reporting it twice is worse than noise: the scanner classifies
      // a mention by resolving it to a PRESENT skill, so in a deliberately trimmed distribution every
      // absent optional sibling also looks "unmentioned" — sk-commander's SKILL.md names
      // sk-jira-footprint at four line numbers, byte-identically, in the core and in this repo, and
      // only the core reported it unused. Worse, the advice differs: "drop the declaration" would
      // delete a real optional edge that the trimmed runtime is expected not to carry
      // (INC-2026-09-04-02, N-8).
      const sibling = entryByName.get(s.skill);
      if (!sibling || !existsSync(sibling.dir)) continue;
      add(
  ...
  // ── 7b. the executability baseline must match disk ───────────────────────────
  //
  // The blind spot this closes: `bundle{}` records CONTENT hashes only, so a script that lost its
  // exec bit hashes identically and every gate stays green. That is not hypothetical — a full
  // export run once published five non-executable scripts with `skill doctor` passing throughout
  // (memory `skill-export-drops-file-mode`), and a skill round-tripping through any Windows
  // checkout loses the bit by construction (INC-2026-09-05-02, X-3).
  //
  // Reported only where the answer is knowable. On Windows NTFS has no execute bit, so the check
  // would fire on every recorded path and mean nothing; there it is a single notice saying the
  // host cannot answer, which is the honest report and not a pass.
  if (m && Object.keys(m.modes || {}).length) {
    const byRel = new Map(scan.files.map((f) => [f.rel, f.abs]));
    if (process.platform === 'win32') {
      add(
        skill, 'mode-unknowable', 'notice',
        `${Object.keys(m.modes).length} recorded executable(s) cannot be checked on Windows — `
        + 'NTFS has no execute bit',
        'run `sidekicks skill verify` on macOS or Linux to check them'
      );
    } else {
      for (const [rel, digits] of Object.entries(m.modes)) {
        if (Number(digits) !== 755) continue;
        const abs = byRel.get(rel);
        if (!abs) continue;             // absence is bundle-stale's finding, not this one's
        let mode;
        try { mode = statSync(abs).mode; } catch { continue; }
        if (mode & 0o111) continue;
        add(
          skill, 'mode-drift', 'error',
          `'${rel}' is recorded executable and is not — it will fail with 'Permission denied'`,
          `chmod +x '${rel}', or 're-record with 'sidekicks skill manifest ${skill} --apply' if it `
          + 'is deliberately no longer executable'
        );
      }
    }
  }
