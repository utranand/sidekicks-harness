...
export function materializeManifests(repoRoot, opts = {}) {
  ...
  const wrote = [];
  const drift = [];
  // Imported skills whose receipt mirror this run refreshed. Deliberately NOT folded into `wrote`:
  // that list is printed as manifest paths, and a receipt is not one.
  const receipts = [];
  ...
  for (const entry of targets) {
    ...
    if (opts.apply) {
      ...
      // KEEP THE HAPPY PATH GREEN. This write moves `manifest_present` and `file_count`, which are
      // mirrored in an imported skill's receipt — so the documented post-import plan (import →
      // manifest --apply → doctor → heal) used to leave `skill registry --check` red, with an
      // instruction ("re-record it") that no verb performed. A CI-shaped check that fails on its
      // own documented happy path is a check people learn to ignore.
      //
      // Only `mirror` moves: `mirror: null` makes recordProfile re-derive it from disk, and every
      // recorded field — `files{}` above all — is carried through untouched.
      //
      // A skill with no receipt gets none written: most skills here were authored in this repo, and
      // inventing a receipt would assert a provenance nobody established. And the whole thing is
      // best-effort — a receipt is bookkeeping, the manifest is the deliverable, so a failure here
      // must never undo a write that already succeeded (the same stance offload.mjs takes).
      try {
        const profile = readProfile(repoRoot, entry.skill);
        if (profile) {
          recordProfile(repoRoot, {
            ...profile,
            mirror: null,
            history: appendHistory(profile, {
              at: nowBangkok(), action: 'manifest', detail: read.relPath,
            }),
          });
          receipts.push(entry.skill);
        }
      } catch { /* bookkeeping never undoes a completed write */ }
    }
  }

  return { results, wrote, drift, receipts };
}
...
export async function run(ctx, args) {
  ...
  const { results, wrote, drift, receipts } = materializeManifests(ctx.repoRoot, {
    skill: name,
  ...
  if (flags.apply) {
    ...
    out.push('');
    if (receipts.length) {
      out.push(`  re-recorded ${receipts.length} import receipt mirror(s): ${receipts.join(', ')}`);
    }
    out.push('');
