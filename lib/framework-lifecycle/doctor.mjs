...
//
// Checks:
//   1. every hook wired in any supported CLI config has a registry entry
...
import { coreDirOf, frameworkRoots, resolveOwned } from '../sk-cli/core-mount.mjs';
import { bodyTexts } from './_body.mjs';
...
export function auditFramework(repoRoot) {
  ...
  const wired = wiredHookScripts(repoRoot);
  ...
  for (const script of wired) {
    if (!registeredScripts.has(script)) {
      findings.push({
        ...
        detail: `a per-CLI hook config wires '${script}', which has no entry in `
  ...
  for (const entry of hookEntries) {
    ...
    if (!wiredSet.has(script)) {
      findings.push({
        check: 'hook-unwired',
        detail: `'${entry.id}' claims script '${script}', which no per-CLI hook config wires`,
      });
    }
    if (script && !resolveOwned(frameworkRoots(repoRoot), script.split('/'))) {
      findings.push({
...
export async function run(ctx, _args) {
  ...
  const out = [
    ...
    `  floor:       ${counts.floor} consent-gated ids, all registered`,
    `  visibility:  ${counts.listed} toggleable entries listed in ${SETTINGS_REL_DIR}/`,
    `  wiring:      ${counts.wired_hooks} hooks wired across per-CLI configs, all registered`,
  ];
