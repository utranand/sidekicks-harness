...
export async function run(ctx, _args) {
  ...
  if (checkOnly) {
    ...
    const clean = [
      ...
      `  floor:   ${before.floor.length} consent-gated ids, deliberately not listed (no persistent disable)`,
    ];
  ...
  out.push(`  floor:  ${after.floor.length} consent-gated ids stay unlisted — no persistent disable`);
