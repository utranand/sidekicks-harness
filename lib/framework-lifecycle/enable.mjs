...
export async function run(ctx, args) {
  ...
  const out = [];
  if (entry.floor) {
    out.push(`${id} is part of the consent-gated safety floor — enabled by default, `
      + 'with no persistent setting to write. A one-action waiver requires explicit user approval.');
  } else {
