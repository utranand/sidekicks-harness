...
//
// Scaffolds projects/<name>/{manifest.yaml, config/.gitignore, docs/, output/, assets/}
...
import { CONFIG_DIR } from '../config-store/paths.mjs';
...
export async function run(ctx, args) {
  ...
  // ── Scaffold config/ ────────────────────────────────────────────
  // A project's configuration is a FOLDER, one file per family, with a git-ignored `<family>.secret.yaml`
  // sibling holding the credentials — the same shape `.sidekicks/config/` has. What used to be written
  // here was a 0-byte `config.yaml`: the pre-family monolith, which `config doctor` reports as
  // `legacy-monolith` the moment the project exists and `config migrate` exists to retire
  // (INC-2026-09-06-06 B-7). A brand-new project should not be born needing a migration.
  //
  // Only the directory and its self-contained ignore rule are created. The family files themselves are
  // written by `sidekicks config sync`/`set` from what the installed skills declare — seeding empty
  // ones here would document blocks this project may never use.
  ensureSecretIgnore(repoRoot, join('projects', name, CONFIG_DIR));
