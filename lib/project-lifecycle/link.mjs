...
//   2. Seed metadata   writes manifest.yaml + config/ INTO the external dir when
...
import { writeAtomic, createDirLink, mkdirp } from '../fs-safety/fsx.mjs';
...
import { SECRET_IGNORE_FILE } from '../config-store/write.mjs';
...
export async function run(ctx, args) {
  ...
  // The linked directory gets the same config/ FOLDER a created project gets. Written directly
  // rather than through `ensureSecretIgnore`, for the same reason the manifest above is: the target
  // may live entirely outside this repo, where the CLI write guard refuses — and that out-of-tree
  // case is exactly why the ignore rule has to live INSIDE config/ instead of in the root .gitignore.
  const configIgnore = join(absTarget, CONFIG_DIR, '.gitignore');
  if (!existsSync(configIgnore)) {
    mkdirp(join(absTarget, CONFIG_DIR));
    writeAtomic(configIgnore, SECRET_IGNORE_FILE);
  }
