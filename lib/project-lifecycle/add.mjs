 ...
 *   manifest.yaml (with remote_source pre-set), config/.gitignore, docs/, output/, assets/
...
function scaffoldProject(projectDir, name, url, repoRoot) {
  ...
  // A project's configuration is a FOLDER (see create.mjs) — only the directory and its
  // self-contained `*.secret.yaml` ignore rule are scaffolded; `config sync` writes the families.
