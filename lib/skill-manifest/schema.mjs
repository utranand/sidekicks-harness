...
/** @returns {{schema: number, skill: string|null, requires: object, entrypoints: object[], bundle: Record<string,string>, modes: Record<string,number>}} */
...
export function validateManifest(obj, skill, label) {
  ...
  // ── modes (DERIVED executability baseline) ────────────────────────────────────
  //
  // WHY THIS IS A SIBLING OF bundle{} AND NOT A FIELD INSIDE IT. `bundle` is a flat path -> sha256
  // map and its value regex is closed; widening it to a pair would invalidate every manifest in
  // every repo that already carries one, and the hashes are the one thing that must not churn.
  // A separate optional key costs nothing to an existing manifest: absent means "this baseline
  // predates mode recording", which is honestly different from "nothing here is executable".
  //
  // ONLY EXECUTABLES ARE RECORDED. 0o644 is the overwhelming majority and recording it would
  // triple the block for no information; a path present here means 755, a path absent means 644
  // ONLY when the block itself is present. That asymmetry is why the block is written whenever a
  // skill has at least one executable and omitted entirely otherwise.
  if (obj.modes !== undefined && obj.modes !== null) {
    if (!isMap(obj.modes)) {
      errors.push(`${label}: 'modes' must be a mapping of path to 755`);
    } else {
      for (const [p, m] of Object.entries(obj.modes)) {
        if (escapesSkill(p)) {
          errors.push(`${label}: modes path '${p}' escapes the skill folder`);
          continue;
        }
        const n = typeof m === 'number' ? m : Number.parseInt(String(m), 10);
        if (n !== 755 && n !== 644) {
          errors.push(`${label}: modes['${p}'] must be 755 or 644`);
          continue;
        }
        manifest.modes[p] = n;
      }
  ...
  // ── distribution (AUTHORED runtime projection exception) ──────────────────────
  //
  // Optional and narrow. Framework publication projects a RUNTIME subset of each skill folder,
  // excluding the top-level development surfaces (improvements/, evals/, tests/) by default. This
  // section is the reviewed escape hatch for the rare skill whose runtime genuinely needs a path
  // under one of them, and for a skill that wants to withhold something else from the runtime.
  //
  // Absent means "the built-in policy applies", which is what every existing manifest means today
  // — so the field is null, not {}: "not declared" and "declared empty" must stay distinguishable.
  // Patterns are skill-relative POSIX globs; an absolute path, a '..' segment or a backslash is a
  // validation error rather than a silently-normalized one, because a pattern that resolves
  // somewhere else is a request nobody can review.
  if (obj.distribution !== undefined && obj.distribution !== null) {
    if (!isMap(obj.distribution)) {
      errors.push(`${label}: 'distribution' must be a mapping`);
    } else {
      for (const key of Object.keys(obj.distribution)) {
        if (key !== 'runtime') {
          errors.push(`${label}: unknown key 'distribution.${key}' — nothing reads it`);
        }
      }
      const runtime = obj.distribution.runtime;
      if (runtime !== undefined && runtime !== null && !isMap(runtime)) {
        errors.push(`${label}: 'distribution.runtime' must be a mapping`);
      } else if (isMap(runtime)) {
        const section = { exclude: [], include: [] };
        for (const key of Object.keys(runtime)) {
          if (key !== 'exclude' && key !== 'include') {
            errors.push(`${label}: unknown key 'distribution.runtime.${key}' — nothing reads it`);
          }
        }
        for (const field of ['exclude', 'include']) {
          ...
          if (!Array.isArray(value)) {
            errors.push(`${label}: 'distribution.runtime.${field}' must be a list of patterns`);
            continue;
          }
          for (const pattern of value) {
            if (typeof pattern !== 'string' || pattern === '') {
              errors.push(`${label}: distribution.runtime.${field} holds a non-string pattern`);
              continue;
            }
            if (pattern.includes('\\')) {
              errors.push(
                `${label}: distribution.runtime.${field} pattern '${pattern}' uses a backslash — `
                + 'patterns are POSIX and skill-relative'
              );
              continue;
            }
            if (escapesSkill(pattern)) {
              errors.push(
                `${label}: distribution.runtime.${field} pattern '${pattern}' escapes the skill folder`
              );
              continue;
            }
            section[field].push(pattern);
          }
        }
        manifest.distribution = { runtime: section };
      }
    }
  }
  ...
  const known = new Set([
    'schema', 'skill', 'requires', 'not_required', 'entrypoints', 'bundle', 'modes',
    'distribution',
  ]);
