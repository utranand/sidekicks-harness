...
import { recordModes } from './mode.mjs';
...
export function manifestPlan(scan, existing, derived, frontmatterDeps = []) {
  ...
  // The executability baseline, alongside the content baseline and for the same reason: a property
  // that is not recorded cannot be verified or restored. Only executables are listed, so the block
  // is absent for the great majority of skills and its ABSENCE means "nothing here is executable"
  // — but only once the block has been written at least once, which is why `modesChanged` compares
  // against a manifest that may predate mode recording entirely.
  const modes = recordModes(scan.dir, scan.files.filter((f) => f.rel !== MANIFEST_NAME));
  const prevModes = existing ? (existing.modes || {}) : {};
  const modesChanged = JSON.stringify(prevModes) !== JSON.stringify(modes);
  ...
  return {
    ...
    modesChanged,
    derivedChanged,
    changed: !existing || addCount > 0 || bundleChanged || modesChanged || derivedChanged,
  };
}
...
export function renderManifest(plan, existingDegraded = null) {
  ...
  // Written only when the skill HAS an executable. An empty block would say "nothing is
  // executable" in a file whose absence already says the same thing less noisily.
  if (Object.keys(plan.modes || {}).length) {
    L.push('');
    L.push('# DERIVED executability baseline — do not hand-edit. Only executables are listed;');
    L.push('# a path here lands 0755, any other bundled path lands 0644. Recorded because a');
    L.push('# Windows checkout cannot report an exec bit, so a copy that passes through one');
    L.push('# loses it silently and no content hash can notice.');
    L.push('modes:');
    for (const rel of Object.keys(plan.modes).sort()) {
      L.push(`  ${scalar(rel)}: ${plan.modes[rel]}`);
    }
  }
  L.push('');
  return L.join('\n');
...
export function upsertManifest(text, plan) {
  ...
  if (plan.modesChanged) {
    // An empty replacement REMOVES the block, which is the right answer when a skill's last
    // executable was dropped: leaving a stale `modes:` behind would keep asserting a bit that no
    // file carries any more, and `mode-drift` would then be reported for ever.
    replaceBlock('modes', 0, null, Object.keys(plan.modes || {}).length
      ? [
        ...
        'modes:',
        ...Object.keys(plan.modes).sort().map((rel) => `  ${scalar(rel)}: ${plan.modes[rel]}`),
      ]
      : []);
  }
