...
export async function run(ctx, args) {
  const { repoRoot } = ctx;
  // Derive these from raw argv so a space-form global flag value (for example
  // `--host-cli codex`) can never leak into the sub-action or its operands.
  const positionals = cleanPositionals(ctx.argv);
  const action = positionals[2];         // undefined (show) | "set" | "clear" | "skill"
  const names = positionals.slice(3);    // executor names for `set`, sub-args for `skill`
  ...
  if (!action) {
    ...
    const lines = [];
    if (chain.length === 0) {
      const natural = executorOrder(effective, { hostCli: globalFlags['host-cli'] })
        .filter((name) => effective[name].enabled !== false);
      lines.push(`no auto-routing policy set (${pathRel}).`);
      lines.push(`  \`--executor auto\` uses the current host when eligible, then the first AVAILABLE enabled executor in stable neutral order:`);
