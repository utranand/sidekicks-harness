...
//   - root project `sidekicks` (default) → `.sidekicks/config/cli-executors.json`
//   - user project `<active>`            → `projects/<active>/config/cli-executors.json`
// frameworkConfigPath prefers `config/` and falls back to the pre-move top-level name, so an
// unmigrated checkout keeps resolving.
...
// Complexity tiers an item can request (AGENTS.md → "Subagent model selection — resolve by tier,
...
// Model ids are scope configuration, so the orchestrator picks a tier by task complexity and the
// executor resolves tier→id at invoke time. An absent/empty map means "no model
// flag" → the CLI's own default, exactly the pre-feature behavior.
// `top` is the Fable/Mythos-class rung ABOVE high (AGENTS.md → "Fable fallback"): map it only where
...
export function readRegistry(path) {
  ...
  const rawExecutors = (parsed && typeof parsed.executors === 'object' && !Array.isArray(parsed.executors) && parsed.executors) || {};
  const executors = {};
  for (const [name, spec] of Object.entries(rawExecutors)) {
    if (!spec || typeof spec !== 'object' || Array.isArray(spec)) {
      throw new SidekicksError(`cli-executor: registry ${path} executor '${name}' must be an object`, EXIT_VALIDATION);
    }
    const models = validateModels(name, spec.models);
    executors[name] = { ...spec };
    if (models) executors[name].models = models;
    else delete executors[name].models;
  }
...
/**
 * Resolve an optional current-host hint. The project never persists a host as its default:
 * callers may pass `--host-cli`, while CLI ports can set SIDEKICKS_HOST_CLI for their process.
 * An unknown hint is ignored here so an older Sidekicks checkout remains usable from a newer host.
 */
export function currentHostCli(explicit = '', env = process.env) {
  const detected = [
    ['antigravity', env.AGENT_PROJECT_DIR],
    ['claude', env.CLAUDE_PROJECT_DIR],
    ['gemini', env.GEMINI_PROJECT_DIR],
  ].find(([, marker]) => typeof marker === 'string' && marker.trim() !== '');
  const value = String(explicit || env.SIDEKICKS_HOST_CLI || detected?.[0] || '').trim();
  return NAME_RE.test(value) ? value : '';
}

/** Provider-neutral candidate order: eligible current host, explicit local policy, stable slug. */
export function executorOrder(executors, { hostCli = '', prefer = [] } = {}) {
  const host = currentHostCli(hostCli);
  return [...new Set([
    ...(host && executors[host] ? [host] : []),
    ...prefer,
...
export function validateModels(name, models) {
  ...
  for (const tier of MODEL_TIERS) {
    ...
    if (!MODEL_ID_RE.test(val)) {
      throw new SidekicksError(
        `cli-executor: '${name}' models.${tier} must match ${MODEL_ID_RE.source}`,
        EXIT_VALIDATION
      );
    }
    out[tier] = val;
...
/**
 * Grade one executor's selected model per tier against its discovered capability catalog. This is
 * the ONE place the valid/stale/unverified/unmapped rule lives — `list` renders it and `models`
 * gates on it, so a second copy would be a second answer.
 *
 * - `unmapped`    — no model is selected for that tier (never a defect; a tier a CLI does not offer)
 * - `unverified`  — a model is selected but no COMPLETE catalog exists to check it against
 * - `stale`       — the selected model (or its effort) is absent from a complete catalog
 * - `valid`       — the selected model, and any selected effort, appear in the catalog
 *
 * @param {Record<string, any>} spec one executor's resolved spec
 * @returns {Record<string, 'valid'|'stale'|'unverified'|'unmapped'>} keyed by tier
 */
export function selectionStatus(spec) {
...
export function effectiveExecutors(registry) {
  ...
  for (const [name, spec] of Object.entries(registry.executors || {})) {
    const merged = { name, ...(out[name] || {}), ...spec };
    const models = validateModels(name, merged.models);
...
export function effectiveRegistry(rootRegistry = {}, projectRegistry = null) {
  ...
  for (const [name, spec] of Object.entries(effective)) {
    const models = validateModels(name, spec?.models);
    if (models) spec.models = models;
    else delete spec.models;
  }
