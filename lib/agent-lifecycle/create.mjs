...
export async function run(ctx, args) {
  ...
  const cli = flags.cli ? String(flags.cli) : '';
  if (cli && !AGENT_CLIS.includes(cli)) {
    throw new SidekicksError(
  ...
  // A CLI pin is agent-specific and optional. Unpinned charters stay portable and resolve through
  // the effective executor registry when started or delegated.
  if (cli) charter.cli = cli;
