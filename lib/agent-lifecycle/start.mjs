...
import {
  ...
  routingPolicy,
} from '../cli-executor-lifecycle/_shared.mjs';
import {
  parseMemoryFlags,
  validateAgentName,
  assertModelId,
 ...
 * @param {string} cliName - the resolved executor name.
 * @param {object} executorSpec - effective registry entry for that executor.
 * @returns {{ resolved: string, tier: string|null }|null}
 */
export function resolveModel(modelSpec, cliName, executorSpec = null) {
  ...
  // The executor registry is the only tier→concrete-model authority. The optional lookup keeps
  // direct callers safe: built-in structure is known, but no concrete model is guessed when the
  // caller did not supply its effective configured row.
  const spec = executorSpec || effectiveExecutors({ executors: {} })[cliName];
  const resolved = spec?.models?.[tier];
  if (!resolved) {
  ...
  assertModelId(resolved, (message) => {
    throw new SidekicksError(`agent start: configured tier mapping is invalid: ${message}`, EXIT_VALIDATION);
  });
  return { resolved, tier };
}

/** Build the child delegate argv without losing one-off CLI or host routing hints. */
export function headlessDelegateArgs(name, flags = {}, hostCli = '') {
  const args = ['agent', 'delegate', name];
  if (flags.cli != null) args.push('--cli', String(flags.cli));
  if (hostCli) args.push('--host-cli', String(hostCli));
  if (flags.model != null) args.push('--model', String(flags.model));
  if (flags['permission-mode']) args.push('--permission-mode', String(flags['permission-mode']));
...
export async function run(ctx, args) {
  ...
  const modelTier = TIERS.includes(modelSpec.toLowerCase()) ? modelSpec.toLowerCase() : '';
  const registry = readEffectiveRegistry(repoRoot, readSettings(repoRoot));
  const executors = effectiveExecutors(registry);
  const prefer = routingPolicy(registry);
  const hostCli = currentHostCli(flags['host-cli']);
  ...
  // antigravity` without pinning molly's provider-neutral charter).
  let cliName;
  if (flags.cli) {
    ...
    cliName = resolveCharterCli(charter, name, 'agent start', {
      ...
      hostCli,
      modelTier,
      available: (cli) => commandExists(CLI_LAUNCH[cli].bin),
    });
  }
  ...
  const executorSpec = executors[cliName];
  if (!executorSpec || executorSpec.enabled === false) {
    throw new SidekicksError(
      `agent start: cli '${cliName}' is not enabled in the effective executor registry`,
      EXIT_VALIDATION,
    );
  }
  ...
  const modelInfo = resolveModel(modelSpec, cliName, executorSpec);
  ...
  if (flags.headless) {
    ...
    const delegateArgs = headlessDelegateArgs(name, flags, hostCli);
