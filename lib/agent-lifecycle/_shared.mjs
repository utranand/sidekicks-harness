...
import { executorOrder } from '../cli-executor-lifecycle/_shared.mjs';
 ...
 * @param {{executors?: Record<string,object>, prefer?: string[], hostCli?: string, modelTier?: string,
 *          available?: (cli: string) => boolean}} options
 * @returns {string} a name guaranteed to be in AGENT_CLIS
 */
export function resolveCharterCli(charter, name, verb, options = {}) {
  const raw = charter && charter.cli != null ? String(charter.cli).trim() : '';
  if (raw && AGENT_CLIS.includes(raw)) return raw;
  if (!raw) {
    const executors = options.executors || Object.fromEntries(AGENT_CLIS.map((cli) => [cli, { enabled: true }]));
    for (const cli of executorOrder(executors, { hostCli: options.hostCli, prefer: options.prefer })) {
      const spec = executors[cli];
      if (!AGENT_CLIS.includes(cli) || !spec || spec.enabled === false) continue;
      if (options.modelTier && !spec.models?.[options.modelTier]) continue;
      if (options.available && !options.available(cli)) continue;
      return cli;
    }
    throw new SidekicksError(
      `${verb}: no enabled agent CLI can${options.modelTier ? ` map model tier '${options.modelTier}'` : ' launch this agent'}`,
      EXIT_VALIDATION,
    );
  }
  ...
  throw new SidekicksError(
    ...
    + `Edit .sidekicks/agents/${name}/agent.yaml and set 'cli:' to one of: ${AGENT_CLIS.join(', ')}, or remove it to inherit the current host`,
    EXIT_VALIDATION
  );
}
...
export function validateCompleteCharter(charter, expectedName, verb = 'agent', { deferCliValidation = false, deferModelValidation = false } = {}) {
  ...
  if (!deferCliValidation && charter.cli != null && String(charter.cli).trim() !== '' && !AGENT_CLIS.includes(String(charter.cli).trim())) {
    fail(`cli must be one of: ${AGENT_CLIS.join(', ')}, or omitted to inherit the current host`);
  }
