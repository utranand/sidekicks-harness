...
export function readPack(dir, dirName, origin = 'workspace') {
  ...
  try {
    rec.manifest = validatePackManifest(parsed, dirName, listPackAgentDirs(dir));
  } catch (err) {
...
/**
 * Every agent directory that actually carries a charter, sorted.
 *
 * This is what makes a pack grow by itself: a charter dropped into `agents/` is either shipped (when
 * the manifest omits `agents:` entirely) or refused by name (when the manifest lists a different
 * set). What it is never is silently ignored — a pack whose folder and manifest disagree is the one
 * failure mode nobody notices, because the agent is right there on disk.
 *
 * @param {string} packDir absolute path of the pack directory
 * @returns {string[]}
 */
export function listPackAgentDirs(packDir) {
  let entries;
  try {
    entries = readdirSync(join(packDir, PACK_AGENTS_DIR), { withFileTypes: true });
  } catch {
    return [];                         // an absent agents/ dir is reported by validation, not here
  }
  return entries
    .filter((e) => (e.isDirectory() || e.isSymbolicLink()) && existsSync(join(packDir, PACK_AGENTS_DIR, e.name, 'agent.yaml')))
    .map((e) => e.name)
    .sort();
}
 ...
 * @param {string[]|null} [onDisk] the agent directories found on disk. When given, `agents:` may be
 *   omitted and is derived from it, and a manifest that DOES list agents must match it exactly.
 *   Omit (or pass null) to validate a manifest that has no directory behind it yet.
 * @returns {PackManifest}
 * @throws {SidekicksError(EXIT_VALIDATION)}
 */
export function validatePackManifest(manifest, expectedId, onDisk = null) {
  const fail = (message) => {
  ...
  // `agents:` is DERIVED when the manifest omits it and the directory is there to answer the
  // question — so adding an agent to a pack is copying a charter in, with no second edit that can be
  // forgotten. Spelling it out stays legal, and then it must agree with the folder.
  const declaredAgents = 'agents' in manifest && manifest.agents != null;
  if (!declaredAgents && Array.isArray(onDisk)) {
    if (!onDisk.length) fail(`no agents — ${PACK_AGENTS_DIR}/<name>/agent.yaml is what defines one`);
    manifest = { ...manifest, agents: [...onDisk] };
  }
  if (!Array.isArray(manifest.agents) || manifest.agents.length === 0) fail('agents must be a non-empty list');
  if (declaredAgents && Array.isArray(onDisk)) {
    const listed = new Set(manifest.agents.filter((a) => typeof a === 'string'));
    const unlisted = onDisk.filter((a) => !listed.has(a));
    if (unlisted.length) {
      fail(
        `${PACK_AGENTS_DIR}/ carries ${unlisted.map((a) => `'${a}'`).join(', ')} that agents: does not list — ` +
        `add the name, or drop the directory (omit agents: entirely to derive it from the folder)`
      );
    }
  }
  const seen = new Set();
