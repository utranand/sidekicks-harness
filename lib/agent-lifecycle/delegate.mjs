...
import { commandExists, resolveModel } from './start.mjs';
...
export function buildHeadlessCommand({ cliName, prompt, model, permissionMode, resume, maxRuntimeMs = null, fullContext = false }) {
  ...
  if (cliName === 'antigravity') {
    ...
    // their suffix (…-high/-medium/-low), as declared by the executor registry tier map.
...
export async function run(ctx, args) {
  ...
  const modelTier = ['top', 'high', 'mid', 'low'].includes(modelSpec.toLowerCase())
  ...
  if (flags.cli != null) {
    ...
    if (!AGENT_CLIS.includes(cliName)) {
      throw new SidekicksError(
        `agent delegate: invalid --cli '${cliName}' — one of: ${AGENT_CLIS.join(', ')}`,
        EXIT_VALIDATION,
      );
    }
  } else {
    cliName = resolveCharterCli(charter, name, 'agent delegate', {
      executors,
  ...
  if (!executorSpec || executorSpec.enabled === false) {
    throw new SidekicksError(
      `agent delegate: cli '${cliName}' is not enabled in the effective executor registry`,
      EXIT_VALIDATION,
    );
  }
  let primaryBinding;
  ...
  try {
    for (;;) {
      ...
      if (pendingBeforeIds.length > 0) {
        ...
        let basePrompt = delegatePrompt(name, {
          fresh,
          handoffRel: briefAvailable ? handoffRel : null,
        });
        const privateSkills = privateSkillContext(repoRoot, charter, basePrompt);
        basePrompt += `\n\n${composePrimaryPrompt(charter, primaryBinding, privateSkills)}`;
