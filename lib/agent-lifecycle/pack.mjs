...
// `sidekicks agent pack <list|show|install|status|harvest> [<pack-id>] [--dry-run] [--json]`
//
...
//
// `harvest` is the opposite direction — live agents OUT into a portable pack, so a crew an operator
// actually uses can ship inside a framework core. It refuses a charter it cannot make portable
// rather than editing the operator's prose; see ./_harvest.mjs.
//
...
import { harvestPack } from './_harvest.mjs';

const SUBS = ['list', 'show', 'install', 'status', 'harvest'];
...
export async function run(ctx, args) {
  ...
  if (sub === 'harvest') return cmdHarvest(repoRoot, requireId(sub, packId), flags);
...
/**
 * `agent pack harvest <pack-id> --agents a,b [--version X.Y.Z] [--display-name s] [--summary s]
 *                               [--skills-repo <url>] [--dry-run] [--json]`
 *
 * Live agents OUT into a portable pack. The refusal path is the point: an agent whose charter names
 * the Telegram lane, the pacemaker or an absolute path is reported by field path and left alone,
 * because rewriting an operator's prose to pass a gate publishes an agent that no longer describes
 * what it does.
 */
function cmdHarvest(repoRoot, packId, flags) {
  const agents = splitList(flags.agents ?? '');
  const plan = harvestPack(repoRoot, packId, {
    agents,
    version: flags.version || undefined,
    displayName: flags['display-name'] || undefined,
    summary: flags.summary || undefined,
    skillsRepo: flags['skills-repo'] || undefined,
    dryRun: Boolean(flags['dry-run']),
  });

  if (flags.json) return { stdout: JSON.stringify(plan, null, 2) + '\n', exitCode: EXIT_OK };

  const out = [];
  const what = plan.created ? 'created' : `updated (v${plan.previous_version} -> v${plan.version})`;
  out.push(`agent pack harvest ${plan.pack} — ${plan.dry_run ? 'DRY RUN, nothing written' : what}`);
  out.push('');
  for (const a of plan.agents) {
    out.push(`  ${a.name}`);
    for (const d of a.dropped) out.push(`      dropped   ${d}`);
    out.push(`      skills    ${a.skills_named.length ? a.skills_named.join(', ') : '(none named in the charter)'}`);
  }
  if (plan.carried_agents.length) {
    out.push('');
    out.push(`  carried unchanged: ${plan.carried_agents.join(', ')}`);
  }
  out.push('');
  out.push(`requires_skills (${plan.requires_skills.length}): ${plan.requires_skills.join(', ') || '(none)'}`);
  if (plan.skills_from_closure.length) {
    out.push(`  added by declared dependencies: ${plan.skills_from_closure.join(', ')}`);
  }
  out.push('');
  out.push(`${plan.dry_run ? 'would write' : 'wrote'} ${plan.files.length} file(s):`);
  for (const f of plan.files) out.push(`  ${f}`);
  if (!plan.dry_run) {
    out.push('');
    out.push(`Check it with: sidekicks agent pack status ${plan.pack}`);
  }
  return { stdout: out.join('\n') + '\n', exitCode: EXIT_OK };
}
