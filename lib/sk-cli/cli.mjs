...
export async function main(argv) {
  ...
  // `--version` is also legitimate verb vocabulary (for example an immutable
  // private-skill release). Treat it as the global shortcut only when no
  // namespace/verb positionals are present; verb lifecycles re-parse valued
  // flags from ctx.argv.
  ...
  //
  // SKIPPED FOR THE UPDATE HANDOFF. `core update` re-enters the newly mounted core as
  // `core update --finish` (lib/core-lifecycle/_finish.mjs), and that process's very next act is the
  // full reconciling overlay pass — so this additive one is duplicated work, and on the FAILURE path
  // it is actively harmful: it creates links for the new core's skills without the managed
  // .gitignore entry that records them, and reconcileCoreSkillOverlay refuses to prune a link it has
  // no record of creating. The rollback then could not clear them and left the workspace with
  // dangling links into a core that had been checked back out (INC-2026-09-04-03).
  if (process.env.SIDEKICKS_CORE_UPDATE_FINISH !== "1") {
    ensureCoreSkillOverlay(repoRoot, log);
  }
  ensureSkillLinks(repoRoot, log);

  // Self-heal the memory merge driver, for the same reason and with the same contract: the
  // `.gitattributes` lines that route the store to it are COMMITTED, but the driver they name
  // lives in .git/config, which is per-clone and never committed. Left unregistered, a memory
  // merge falls back to git's text merge and conflicts in all three generated faces at once —
  // in the clone least likely to know why. Already-registered costs one readFileSync; the
  // single `git config` spawn happens once per clone (worktrees share it). Never throws, and
  // SIDEKICKS_MEMORY_MERGE=off opts out.
  //
  // GATED ON THE STORE BEING TRACKED. It is git-ignored in a default checkout, and git never merges
  // an untracked path — so there is nothing for the driver to do, and registering it would spawn
  // `git config` on every run of every verb for a surface that does not exist. A checkout that DOES
  // track its store (a memory source repository) keeps the self-heal unchanged.
  try {
    const { ensureMemoryMergeDriver, storeTravelsInGit } = await import("../memory-lifecycle/_merge-driver.mjs");
    if (storeTravelsInGit(repoRoot)) ensureMemoryMergeDriver(repoRoot, log);
  } catch (err) {
    log(`memory merge driver self-heal skipped: ${err.message}`);
  }
