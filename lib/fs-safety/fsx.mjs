...
export function writeAtomic(absPath, content, opts) {
  ...
  try {
    renameWithRetry(tmpPath, absPath);
  } catch (err) {
...
/** Errors that mean "someone else has this file open right now", not "this will never work". */
const TRANSIENT_RENAME = Object.freeze(new Set(['EPERM', 'EACCES', 'EBUSY', 'ENOTEMPTY']));

/**
 * `renameSync`, retried through the Windows failure that is not a failure.
 *
 * On POSIX a rename over an existing file always succeeds. On Windows it fails with EPERM or EBUSY
 * whenever ANYTHING holds a handle on the destination — an editor with the file open, a search
 * indexer, an antivirus scanner mid-scan — and all three are transient by nature. Without a retry
 * an import stops half a folder in for a reason that would have gone away by itself, which is the
 * failure `applyOne`'s staging exists to make recoverable and this makes rare (INC-2026-09-05-02,
 * X-4).
 *
 * Deliberately short and bounded: ~10ms, ~20ms, ~40ms. Long enough to outlast a scanner, short
 * enough that a genuinely locked file still reports promptly rather than hanging a CLI verb. The
 * loop is a busy wait because there is no synchronous sleep in node: and every caller of
 * writeAtomic is synchronous.
 *
 * @param {string} from
 * @param {string} to
 */
function renameWithRetry(from, to) {
  ...
  for (let attempt = 0; ; attempt++) {
    try {
      ...
      if (attempt >= delays.length || !TRANSIENT_RENAME.has(err.code)) throw err;
      const until = Date.now() + delays[attempt];
      while (Date.now() < until) { /* spin briefly; see above */ }
    }
  }
}
