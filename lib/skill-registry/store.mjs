...
export function profileDrift(repoRoot) {
  ...
  for (const [name, p] of profiles) {
    ...
    const mismatch = mirrorMismatch(now, p.mirror);
    if (mismatch.length) {
      rows.push({
        skill: name, status: 'tree-drift', fields: mismatch,
...
/**
 * Which mirrored fields disagree with disk.
 *
 * Shared by `--check` (which reports them) and the re-record writer (which fixes them), so the
 * question "what moved?" cannot be answered two different ways by the check and its own remedy.
 * NOT exported — the drift row carries the answer instead, because this module's contract is that
 * it exposes no accessor for a fact already derivable (see the export lock in
 * tests/skills/skill-registry.test.mjs).
 *
 * @param {object} now - mirrorFacts() output
 * @param {object|null|undefined} recorded - the profile's mirror block
 * @returns {string[]}
 */
function mirrorMismatch(now, recorded) {
  const was = recorded && typeof recorded === 'object' ? recorded : {};
  return Object.keys(now).filter((k) => String(now[k]) !== String(was[k]));
}
