...
/**
 * Pick the executor a role runs on: explicit choice, current host when eligible, configured
 * local policy, then a stable provider-neutral order.
 *
 ...
 *
 * @param {{executors: Record<string, object>, prefer?: string[], hostCli?: string, requested?: string|null,
...
export function selectExecutor(input) {
  ...
  const order = executorOrder(executors, { hostCli: input.hostCli, prefer: input.prefer });
  /** @type {string[]} */
  const rejected = [];
 ...
 *
 * @param {{executors: Record<string, object>, prefer?: string[], hostCli?: string, role: string, tier: string,
...
export function selectIndependentExecutor(input) {
  const order = executorOrder(input.executors, { hostCli: input.hostCli, prefer: input.prefer });
