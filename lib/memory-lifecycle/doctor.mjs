...
//
// Six checks, each earning its place from a specific failure this design can produce:
//
...
import { buildIndex, buildGraph, readIndexJson, readGraphJson } from './_store.mjs';
...
export async function run(ctx, _args) {
  ...
  const expected = {
    [machineIndexPath(repoRoot)]: JSON.stringify(buildIndex(entries), null, 2) + '\n',
    [graphPath(repoRoot)]: JSON.stringify(buildGraph(entries), null, 2) + '\n',
