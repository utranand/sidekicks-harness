...
import { driverStatus, storeTravelsInGit, DRIVER_NAME } from './_merge-driver.mjs';
import { readSources } from './_sources.mjs';
...
export async function run(ctx, _args) {
  ...
  //
  // Only asked when the store is TRACKED. Git never merges an ignored path, so in a default
  // checkout every finding below would be a true statement about a surface that does not exist —
  // and a doctor that reliably reports non-problems is a doctor nobody reads.
  const merge = driverStatus(repoRoot);
  const tracked = storeTravelsInGit(repoRoot);
  if (tracked && merge.git && !merge.registered) {
    findings.push({
      check: 'merge-driver',
      severity: 'info',
  ...
  if (tracked && merge.git && !merge.attributes) {
    findings.push({
      check: 'merge-driver',
      severity: 'info',
  ...
  const hooksLabel = merge.hooksDir
    ...
    : '.git/hooks';
  for (const h of (tracked ? merge.hooks : []).filter((x) => x.state === 'foreign')) {
    findings.push({
      check: 'merge-driver',
      severity: 'info',
  ...
  for (const name of (tracked ? merge.strayHooks ?? [] : [])) {
    findings.push({
      check: 'merge-driver',
      severity: 'warn',
  ...
  // 9. TRANSPORT. With the store git-ignored, nothing carries it off this machine on its own. A
  // store with entries and no registered source is one disk failure from gone; an empty store WITH
  // sources is a fresh clone that has simply not hydrated yet. Both are actionable, and neither is
  // visible from any other check.
  //
  // INFO, not warn, for the same reason `dormant-legacy` is: a warning fails this verb (exit 2), and
  // a doctor that goes red on every brand-new store — before anyone has had a chance to register
  // anything — is a doctor people learn to skip. The empty-store half is also surfaced loudly where
  // it actually matters, by the session-start hook (scripts/load-local-memory-hook.mjs).
  if (!tracked) {
    const registered = readSources(repoRoot).sources;
    if (entries.length && !registered.length) {
      findings.push({
        check: 'no-transport',
        severity: 'info',
        detail: `the store holds ${entries.length} entries, is git-ignored, and no external source `
          + `is registered — nothing carries it to another checkout or machine. Register one: `
          + `'sidekicks memory source add <name> --kind dir --path <folder>' (or --kind git --url <repo>), `
          + `then 'sidekicks memory publish <name>'`,
      });
    } else if (!entries.length && registered.length) {
      findings.push({
        check: 'no-transport',
        severity: 'info',
        detail: `the store is empty but ${registered.length} source(s) are registered `
          + `(${registered.map((s) => s.name).join(', ')}) — this checkout has not hydrated yet. `
          + `Run 'sidekicks memory sync'`,
      });
    }
  }
