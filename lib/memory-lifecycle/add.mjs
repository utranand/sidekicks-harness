 ...
 * it may add constraints for the agent, never persist or grant a consent-gated floor waiver. Without this,
 * `memory add protected-branches --rule` would let a data file appear to restate — and
 * therefore appear to relax — a rule only the user may waive for one disclosed action.
 *
 * @param {string} slug
 */
    ...
    if (slug === id || slug === bare) {
      throw new SidekicksError(
        `memory add: '${slug}' collides with the safety-floor id '${id}' — a rule entry can `
          + `ADD constraints, never restate a floor rule or grant its one-action waiver. Choose another slug.`,
        EXIT_VALIDATION
      );
    }
  }
