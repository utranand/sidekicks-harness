...
/**
 * Locale-INDEPENDENT string order for every sort that feeds a COMMITTED memory face.
 *
 * The generated faces (index.json, graph.json, MEMORY.md) must be byte-identical on every host, or
 * the "regeneration is a no-op against the committed files" gate passes on one machine and fails on
 * another. `String.prototype.localeCompare` orders by the runtime's ICU locale — a th_TH host sorts
 * `bmad-developer` ahead of `bmad-dev-worktree` (hyphen treated as secondary) while a C host sorts
 * the hyphen first — so it is exactly the wrong tool for a file under version control. UTF-16
 * code-unit comparison is fixed across engines, locales and platforms.
 *
 * @param {string} a
 * @param {string} b
 * @returns {number}
 ...
 * @returns {string}
 */
export function buildEntryFile({ name, description, type, created, body, category, rule, source, links, mergeReview }) {
  ...
  // Set by the merge driver when it had to UNION two divergent bodies — the durable
  // "a human should read this" flag. It stays in the file (and therefore in the merge
  // commit) until `memory resolve <slug> --accept` clears it, which is the only way a
  // reviewer's judgement outlives the session that merged the branch.
  if (mergeReview) metadata.merge_review = mergeReview;
...
/**
 * Does this text carry git conflict markers?
 *
 * Cheap and deliberately anchored to line starts: a `<<<<<<<` inside prose or a fenced
 * code block is not a conflict, and a false positive here would send a reader to
 * `memory resolve` for a file that is fine.
 *
 * @param {string} text
 * @returns {boolean}
 */
export function hasConflictMarkers(text) {
  return /^<{7}(?: |\t|$)/m.test(String(text ?? '').replace(/\r\n?/g, '\n'));
}

/**
 * Split a conflict-marked file back into its sides.
 *
 * Both marker shapes are handled because both reach the store: the default two-way
 * form (`<<<<<<<` / `=======` / `>>>>>>>`) and diff3 (`<<<<<<<` / `|||||||` /
 * `=======` / `>>>>>>>`), which is what `merge.conflictStyle=diff3` and our own driver's
 * fallback produce. `base` is null for the two-way form — there is nothing to recover.
 *
 * Text outside any conflict hunk belongs to every side, so the three returned texts are
 * complete files, not fragments: they can be parsed by parseEntryFile as-is.
 *
 * @param {string} text
 * @returns {{ ours: string, theirs: string, base: string|null, hunks: number }}
 */
export function parseConflictSides(text) {
  ...
  let sawBase = false;
  let hunks = 0;
  // 'both' outside a hunk; then 'ours' → optional 'base' → 'theirs'.
  let where = 'both';
  for (const line of lines) {
    if (/^<{7}(?: |\t|$)/.test(line)) { where = 'ours'; hunks += 1; continue; }
    if (where !== 'both' && /^\|{7}(?: |\t|$)/.test(line)) { where = 'base'; sawBase = true; continue; }
    if (where !== 'both' && /^={7}$/.test(line)) { where = 'theirs'; continue; }
    if (where !== 'both' && /^>{7}(?: |\t|$)/.test(line)) { where = 'both'; continue; }
    if (where === 'both') { ours.push(line); theirs.push(line); base.push(line); continue; }
    if (where === 'ours') { ours.push(line); continue; }
    if (where === 'base') { base.push(line); continue; }
    theirs.push(line);
  }
  return {
    ours: ours.join('\n'),
...
export function readEntryMeta(absPath, fallbackName, { withBody = false } = {}) {
  ...
  // A conflicted entry must not DEGRADE. Markers inside the frontmatter make yaml.parse
  // throw, frontmatter falls back to {}, and the entry silently loses its category, its
  // lineage and — the dangerous one — `rule: true`: a hard rule stops loading and nothing
  // says so. Reading the OURS side keeps the metadata that this checkout committed, and
  // `conflicted` makes the state reportable (memory doctor → memory resolve).
  const conflicted = hasConflictMarkers(text);
  const { frontmatter, body } = parseEntryFile(conflicted ? parseConflictSides(text).ours : text);
  ...
  const out = {
    ...
    links,
    // Merge state — reported by `memory doctor`, repaired by `memory resolve`.
    conflicted,
    mergeReview: typeof meta.merge_review === 'string' && meta.merge_review ? meta.merge_review : null,
  };
...
export function scanStore(repoRoot, { withBody = false } = {}) {
  ...
  for (const namespace of listNamespaces(repoRoot)) {
    ...
    for (const slug of listEntrySlugs(baseDir)) {
      ...
      // Merge state travels with the scan but NOT into the generated faces — index.json
      // stays a function of the entries' content, so a conflict does not churn it.
      if (meta.conflicted) rec.conflicted = true;
      if (meta.mergeReview) rec.mergeReview = meta.mergeReview;
...
export function rebuildCentralIndexText(repoRoot, entries) {
  ...
  for (const ns of order) {
    ...
    const categories = [...byCategory.keys()].sort(byCodeUnit);
    for (const cat of categories) {
      const items = byCategory.get(cat).sort((a, b) => byCodeUnit(a.slug, b.slug));
