...
//
// MODE. Content is byte-exact; the destination mode is NORMALIZED to 0o755 or 0o644 and handed to
// writeAtomic as `opts.mode`, applied to the temp file before the rename. Normalized, never a raw
// mode copy, so an export stays reproducible across umasks and platforms rather than dragging along
// whatever the source filesystem happened to carry.
//
// WHICH of the two is decided by `mode.mjs resolveSourceMode()`, not by the local stat. The stat
// was the whole answer until 2026-09-05, and it is the wrong one on Windows: NTFS has no execute
// bit, libuv reports one only for `.exe/.cmd/.bat/.com`, and `chmodSync` there silently does
// nothing — so exporting from a Windows checkout published every `.sh` and `.py` as 0644, and
// because `bundle{}` records content hashes only, neither `skill verify` nor `skill doctor` could
// see it. The resolver prefers a recorded `modes{}`, then the source repository's git index (which
// reports 100755 from the OBJECT, not the working tree), then the stat, then a shebang.
//
 ...
 */
export function copyBundle(entry, destSkillDir, files, opts = {}) {
  const copied = [];
  const skipped = [];
  // One git query for the folder, not one per file. Null when the source is not a git worktree,
  // in which case resolveSourceMode falls through to the stat and then the shebang.
  const gitExec = gitExecPaths(entry.dir);
  const recorded = opts.modes && typeof opts.modes === 'object' ? opts.modes : {};
  for (const rel of files) {
    const src = join(entry.dir, ...rel.split('/'));
    if (!existsSync(src)) { skipped.push(rel); continue; }
    // NOT execAwareMode(src): that reads the local stat, and on Windows the local stat cannot
    // report an exec bit at all, so exporting from a Windows checkout published every script as
    // 0644 with no way for the destination to notice (INC-2026-09-05-02, X-3). The recorded
    // modes{} and the source git index both survive that platform; the stat is the fallback.
    writeAtomic(join(destSkillDir, ...rel.split('/')), readFileSync(src), {
      mode: resolveSourceMode(src, { rel, recorded, gitExec }),
    });
    copied.push(rel);
  }
