 ...
 * canonical location Rule 3 gave it in this repo. Layout 1 published to `<root>/.sidekicks/skills/`.
 * The PARKED tree is `<root>/.sidekicks/skill-offloaded/` in BOTH layouts and did not move — the two
 * trees stopped sharing a parent, which is why `publishedTreeFor()` in skill-lifecycle/export.mjs is
 * a map and not a prefix swap.
 *
 ...
 * `DEST_TREES` in skill-lifecycle/destinations.mjs), both of which read either layout. So a layout-1
 * clone published before this bump keeps importing and keeps reporting status, forever, with no
 * migration — the stamp records which layout wrote a tree, it does not gate reading one.
 */
