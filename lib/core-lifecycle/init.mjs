...
//     lib/framework-lifecycle/tests/agent-context-mirror.test.mjs, which a workspace does not have — pointing a workspace at
...
import {
  ...
  emptyMemoryIndex, writeIfAbsent, writeSystem, frameworkDocRel, FRAMEWORK_DOC_NAMES,
} from './_seed.mjs';
 ...
 *
 * A copy is REFRESHED on every later run, which is what makes the fallback honest: a symlink tracks
 * AGENTS.md for free, a copy only tracks it if something rewrites it. Which files are ours to
 * rewrite is recorded (readMirrorRecord), never inferred — a file the workspace owner authored looks
 * exactly like a stale copy from the outside, and guessing wrong would overwrite their work.
 *
 * Exported for tests: the refresh path needs a workspace whose mirror is already a copy, which is a
 * state a POSIX host never reaches on its own.
 *
 * @param {string} repoRoot
 * @returns {{created: string[], copied: string[], refreshed: string[]}}
 */
export function ensureInstructionMirrors(repoRoot) {
  const created = [];
  const copied = [];
  const refreshed = [];
  const owned = readMirrorRecord(repoRoot);
  let agents = null;
  try { agents = readFileSync(join(repoRoot, 'AGENTS.md'), 'utf8'); } catch { /* nothing to mirror */ }
  ...
  for (const name of ['CLAUDE.md', 'GEMINI.md']) {
    ...
    if (st && st.isSymbolicLink()) { owned.delete(name); continue; }   // already mirrored

    if (st) {
      // A REAL FILE. Two very different things wear that shape, and the difference is recorded, not
      // guessed: a file the workspace owner authored, and a COPY this function wrote on a host where
      // symlinkSync failed (Windows without Developer Mode). Before, both hit `continue` — so the
      // copy written on the previous run was skipped forever and drifted from AGENTS.md for good,
      // while `core init`'s own note promised it would be refreshed (INC-2026-09-05-05, W-3).
      // Only a copy WE recorded is rewritten; anything else is still the owner's and left alone.
      if (owned.has(name) && agents !== null) {
        let current = null;
        try { current = readFileSync(linkPath, 'utf8'); } catch { /* unreadable — leave it */ }
        if (current !== null && current !== agents) {
          try { writeAtomic(linkPath, agents); refreshed.push(name); } catch { /* core doctor reports it */ }
        }
      }
      continue;
    }
    ...
    try {
      ...
      try {
        if (agents === null) throw new Error('no AGENTS.md to copy');
        writeAtomic(linkPath, agents);
        copied.push(name);
        owned.add(name);
      } catch { /* nothing more to try — reported by core doctor */ }
    }
  }
  writeMirrorRecord(repoRoot, owned);
  return { created, copied, refreshed };
}

/**
 * Which instruction mirrors this workspace holds as COPIES rather than symlinks.
 *
 * Derived, per-machine and git-ignored — it records a fact about THIS checkout's filesystem
 * (symlinks were unavailable when the mirror was made), which is exactly what `.sidekicks/state/`
 * is for. It deliberately is not a sentinel inside CLAUDE.md itself: the multi-CLI parity suites
 * assert the mirrors are byte-identical to AGENTS.md, so a marker in the file would break the very
 * invariant the mirror exists to hold.
 *
 * A missing or corrupt record is never an error — it just means "nothing is known to be a copy",
 * and a pre-existing copy from before this record existed stays untouched, which is the same
 * conservative answer the old code gave.
 *
 * @param {string} repoRoot
 * @returns {Set<string>}
 */
function readMirrorRecord(repoRoot) {
  try {
    const parsed = JSON.parse(readFileSync(mirrorRecordPath(repoRoot), 'utf8'));
    return new Set(Array.isArray(parsed?.copies) ? parsed.copies.filter((n) => typeof n === 'string') : []);
  } catch {
    return new Set();
  }
}

/** @param {string} repoRoot @param {Set<string>} names */
function writeMirrorRecord(repoRoot, names) {
  const path = mirrorRecordPath(repoRoot);
  // Nothing to remember and nothing remembered: do not create the file just to say "empty".
  if (!names.size && !existsSync(path)) return;
  try {
    mkdirp(join(repoRoot, '.sidekicks', 'state'));
    writeAtomic(path, `${JSON.stringify({ copies: [...names].sort() }, null, 2)}\n`);
  } catch { /* a workspace that cannot write its own state still has working mirrors */ }
}

/** @param {string} repoRoot @returns {string} */
function mirrorRecordPath(repoRoot) {
  return statePath(repoRoot, 'instruction-mirrors.json');
}
...
export async function run(ctx, _args) {
  ...
  if (docRel) {
    ...
    report.notes.push(
      `the core ships none of ${FRAMEWORK_DOC_NAMES.join(', ')} — no framework instruction import `
  ...
  for (const n of mirrors.copied) {
    ...
    // Naming the CAUSE, not just the symptom. On Windows a file symlink needs Developer Mode or an
    // elevated process — directory links dodge this by being junctions, but there is no such thing
    // as a file junction, so the mirrors have no privilege-free equivalent. Without the cause the
    // note reads as an unexplained degradation with no action attached to it.
    report.notes.push(
      `${n} is a COPY, not a link — this host could not create a file symlink. `
      + 'On Windows, enable Developer Mode (Settings > Privacy & security > For developers) and '
      + "re-run 'sidekicks core init' to restore a real link; until then this file is refreshed "
      + 'from AGENTS.md on every core init/update.'
    );
  }
  for (const n of mirrors.refreshed) {
    report.created.push(`${n} (copy refreshed from AGENTS.md)`);
  }
