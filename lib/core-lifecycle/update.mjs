...
export async function run(ctx, _args) {
  ...
  if (supportsFinish(coreDir)) {
    ...
    fin.notes.push(
      // "above", not "below": the doctor is the CLOSING verdict (see doctorLines' doc comment and
      // INC-2026-09-04-03 R-2, which specifies it as the tail), and the notes print after it. The
      // original wording described a layout that never shipped.
      'the newly mounted core predates the update handoff, so THIS core\'s rules finished the '
      + 'update — if the doctor above reports wiring or tracked-ref drift, run '
