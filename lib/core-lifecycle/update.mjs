...
export async function run(ctx, _args) {
  ...
  //
  // Both paths need it, and they are different: `alreadyThere` means the mount is already at the
  // target SHA, so there is no gitlink to stage — but --ref may still have just written a NEW
  // tracked ref, which is exactly the "re-pin an already-correct mount" case that recorded nothing.
  let staged = false;
  const toStage = [];
  if (!alreadyThere) toStage.push(before.coreRel);
