...
export async function run(ctx, args) {
  ...
  // A project-scoped block has no root home, so writing one there would put a value in a file the
  // resolver never reads -- a silent no-op the operator would only discover when the setting had no
  // effect. Refuse and name what to do instead. `--root` does not override it: the flag chooses
  // between two valid homes, and here there is only one.
  if (entry.scope === 'project' && useRoot) {
    throw new SidekicksError(
      `config set: block '${blockName}' is project-scoped, so it has no root-scope home`
      + (projectRelPath === null
  ...
  // PUBLIC KEYS. A key the block declares public is credential-SHAPED and holds none, and declaring
  // it exempts its WHOLE SUBTREE — the same semantics splitSecrets() applies on the read/split side.
  // Without this the routing was name-only, so `cluster_ops.secret_name_map.<svc>.<env>.k8s_secret_name`
  // went to the git-ignored half: sk-cluster-ops declares that map public precisely so the mapping and
  // its "applied under an assumed name" warning stay in git, and this endpoint was taking it back out.
  const publicKeys = new Set(entry.public_keys || []);
  const underPublic = segments.some((segment) => publicKeys.has(segment));
  const toSecret = Boolean(flags.secret) || (!underPublic && SECRET_KEY_RE.test(leaf));
  ...
  if (!toSecret && !underPublic && SECRET_KEY_RE.test(keyPath)) {
    throw new SidekicksError(
