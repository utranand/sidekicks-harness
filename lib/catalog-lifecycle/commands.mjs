...
import { frameworkRoots, resolveOwned, describeRoots, coreDirOf, isCoreCheckout }
  from '../sk-cli/core-mount.mjs';
...
/**
 * Which SHAPE of tree this is -- the question `existsSync('docs')` was standing in for, badly.
 *
 * The mount is checked FIRST and that ordering is the fix. A consumer workspace normally carries its
 * own `docs/`, so the old proxy answered 'source' there and demanded `docs/generated/*` from a
 * project that will never have them; and in a mount it resolved every framework path against the
 * workspace root, reporting 159 dispatch modules and a shipped snapshot as missing
 * (INC-2026-09-04-01, F-3).
 *
 *   mounted  -- a workspace with a framework core at .sidekicks-core/
 *   core     -- a standalone core checkout (a clone, or the service checkout it is forged into)
 *   source   -- the Sidekicks source repo, which carries docs/ and the generated pair
 *   packaged -- a distributed package: no docs/, snapshot only
 *
 * @param {string} repoRoot
 * @returns {'mounted'|'core'|'source'|'packaged'}
 */
export function catalogTreeKind(repoRoot) {
  if (coreDirOf(repoRoot)) return 'mounted';
  if (isCoreCheckout(repoRoot)) return 'core';
  if (existsSync(join(repoRoot, 'docs'))) return 'source';
  return 'packaged';
}
 ...
 *
 * Derived from `catalogTreeKind` rather than re-testing `docs/`, so every caller (including
 * lib/scope-lifecycle/explain-model.mjs) inherits the mount fix instead of keeping its own copy of
 * the wrong proxy.
 */
export function isSourceTree(repoRoot) {
  return catalogTreeKind(repoRoot) === 'source';
}

/**
 * The sections a snapshot inside a core can still speak for.
 *
 * A forged core's snapshot is built in the SOURCE repo, and `core init` then runs
 * `framework sync --prune` inside the core -- so the environment-derived sections legitimately
 * differ from the ones the snapshot shipped with, while the declaration-derived ones (cli,
 * durable_formats) must still match byte for byte. Comparing all of them made `catalog check` red
 * inside every forged core, which is a sibling of F-3 and was true before it.
 */
const ENV_DERIVED_SECTIONS = Object.freeze(['framework', 'skills', 'config', 'executors']);
...
export function auditCatalog(repoRoot) {
  ...
  const json = serializeCatalog(model);
  const markdown = renderCatalogMarkdown(model);
  const kind = catalogTreeKind(repoRoot);
  const source = kind === 'source';
  // Computed ONCE: an audit resolves a couple of hundred framework paths, and re-deriving the mount
  // for each turns one existsSync into two.
  const roots = frameworkRoots(repoRoot);
  const inCore = kind === 'mounted' || kind === 'core';
  ...
  if (!source) {
    // Resolved through the roots, not against repoRoot: under a mount the snapshot travelled fine,
    // it just lives at .sidekicks-core/lib/... . "did not travel" is only true when NEITHER root has
    // it, and the message now names where it looked rather than asserting a cause.
    const snapshotAt = resolveOwned(roots, SNAPSHOT_REL.split('/'));
    const snapshotText = snapshotAt ? readNormalized(snapshotAt.abs) : null;
    if (snapshotText === null) {
      add('missing-generated', SNAPSHOT_REL,
        `'${SNAPSHOT_REL}' exists in neither ${describeRoots(roots)} -- the packaged catalog `
        + 'snapshot did not travel');
    } else {
      let snapshot = null;
      try {
      ...
      if (snapshot) {
        // A core's snapshot speaks for the FRAMEWORK, not for the environment: `core init` runs
        // `framework sync --prune` inside the mount, so framework/skills/config/executors
        // legitimately move after the forge. cli and durable_formats are code-derived and must still
        // match exactly -- which is what keeps this a real check rather than a suppression.
        const omit = inCore
          ? [...ENV_DERIVED_SECTIONS]
          : (model.executors.registry_present ? [] : ['executors']);
        const live = catalogFingerprint(model, { omitSections: omit });
        const packaged = catalogFingerprint(snapshot, { omitSections: omit });
        if (live !== packaged) {
          const why = inCore
            ? ` (environment-derived sections omitted: ${ENV_DERIVED_SECTIONS.join(', ')} are `
              + 're-synced inside a core after the forge)'
            : (omit.length ? ` (executors omitted: ${EXECUTOR_REGISTRY_REL} is absent in this `
              + 'package)' : '');
          add('snapshot-drift', SNAPSHOT_REL,
            `packaged snapshot fingerprint ${packaged} does not match the live packaged `
            + `declarations ${live}${why}`);
        }
      }
    }
  }
  ...
  // All three loops resolve workspace-then-core. Every path they carry is framework-owned: cli
  // modules are synthesized `lib/<ns>-lifecycle/<verb>.mjs`, all twelve durable-format rows name
  // `lib/...`, and a config block's defaults sit under `.agents/skills/<skill>/` which a mount
  // reaches through the overlay -- but `core doctor`'s `overlay complete` check permits a plain
  // directory as well as a link, so the core fallback is required for that one too.
  const missing = (segments) => resolveOwned(roots, segments) === null;
  const nowhere = `exists in neither ${describeRoots(roots)}`;

  for (const c of model.cli.commands) {
    if (missing(c.module.split('/'))) {
      add('dangling-reference', c.id,
        `'${c.id}' is registered in lib/sk-cli/help.mjs but '${c.module}' ${nowhere}, so the `
  ...
  for (const f of model.durable_formats.formats) {
    for (const [field, rel] of [['owner', f.owner], ['reader', f.reader], ['writer', f.writer]]) {
      if (!rel) continue;
      if (missing(rel.split('/'))) {
        add('dangling-reference', f.id, `durable format '${f.id}' names ${field} '${rel}', which ${nowhere}`);
      }
    }
  }
  for (const b of model.config.blocks) {
    if (b.defaults && missing(b.defaults.split('/'))) {
      add('dangling-reference', b.id,
        `config block '${b.block}' names defaults '${b.defaults}', which ${nowhere}`);
    }
  }
