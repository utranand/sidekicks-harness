...
function addRetro(ctx, cfg, rest, flags) {
  ...
  const reuseKey = String(flags['reuse-key'] ?? 'none').trim();
  if (reuseKey !== 'none' && !/^[a-z0-9]+(?:-[a-z0-9]+)*$/.test(reuseKey)) {
    throw new SidekicksError(
      "journal retro add: --reuse-key must be 'none' or an exact kebab-case slug",
      EXIT_VALIDATION
    );
  }
  const reuseSummary = String(flags['reuse-summary'] ?? '').trim();
  if (reuseKey !== 'none' && !reuseSummary) {
    throw new SidekicksError(
      'journal retro add: --reuse-summary is required when --reuse-key names reusable work',
      EXIT_VALIDATION
    );
  }
  ...
  const content = buildEntry(
    ...
    [
      ...
      ['Reusable procedure', reuseSummary],
    ]
  );
