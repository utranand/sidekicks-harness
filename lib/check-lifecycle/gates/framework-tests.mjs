...
// which is exactly what a release gate exists to catch. (Both sets were moved there for that reason,
// the contract suites first and the parity suites after they were found naming repo-root `tests/`,
// which is not part of the forged surface at all: the gate could not have run in a mount however its
// paths were resolved.) Reporting `skipped` would be worse than useless here: a blocking gate that
// skips fails the profile anyway, so a skip is a failure that has lost its explanation.
//
...
/**
 * The Rule 6 parity suites. Colocated under `lib/` so they travel, like the contract suites above.
 *
 * They used to live in repo-root `tests/`, which is in no copy plan — absent from the forge's
 * CORE_SURFACES and an explicit member of the packager's FIXED_EXCLUDES. So this gate named two
 * files a mounted workspace could not have, `check run full` was RED in every consumer install, and
 * the failure message below still told them their framework was incomplete (INC-2026-09-04-02, N-3).
 * A core ships per-CLI wiring; whether that wiring is consistent is exactly the question its
 * consumer must be able to ask.
 */
export const PARITY_SUITES = Object.freeze([
  join('lib', 'framework-lifecycle', 'tests', 'multi-cli-parity.test.mjs'),
  join('lib', 'framework-lifecycle', 'tests', 'agent-context-mirror.test.mjs'),
]);
