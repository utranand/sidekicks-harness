...
export async function coreMounted({ repoRoot, spawn, timeoutMs, signal }) {
  ...
  try {
    ...
    cpSync(join(repoRoot, 'package.json'), join(coreRepo, 'package.json'));
    ...
    for (const f of ['rules.yaml', 'criteria.yaml', 'hooks.yaml']) {
      const src = join(repoRoot, '.sidekicks', 'config', 'settings', f);
    ...
    for (const s of CORE_SKILLS) {
      const src = join(repoRoot, '.agents', 'skills', s);
    ...
    if (existsSync(join(repoRoot, 'AGENTS.md'))) {
      cpSync(join(repoRoot, 'AGENTS.md'), join(coreRepo, 'AGENTS.framework.md'));
    }
