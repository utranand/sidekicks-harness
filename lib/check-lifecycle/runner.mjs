 ...
 *
 * `skipped` and `not_applicable` are DIFFERENT answers and the difference is load-bearing.
 *
 *   skipped         the gate could have run here and did not. That is a gap, so a blocking gate
 *                   that skips fails the profile — a skip is not a pass and may never read as one.
 *   not_applicable  the gate has no subject in this environment, by construction. That is a fact
 *                   about the environment, not a gap, so it passes the profile and lets its
 *                   dependents run.
 *
 * Exactly one gate needs the second answer today: `golden.replay` in a mounted workspace. Its
 * fixtures are source-repo contract snapshots — a consumer has nothing to compare them against, and
 * `scripts/update-golden-contracts.mjs` must never run there — so they are deliberately not part of
 * the forged surface. Collapsing that into `skipped` made `check run release` permanently red in
 * every consumer install for a gate that had correctly concluded it had nothing to do
 * (INC-2026-09-04-02, N-3). Collapsing it into `passed` would be a lie of a different kind.
 */
export const GATE_STATUS = Object.freeze(['passed', 'failed', 'blocked', 'skipped', 'not_applicable']);

/** Statuses that let a dependent gate run, and that do not fail a profile. */
const CLEARS = Object.freeze(['passed', 'not_applicable']);
 ...
 */
/** The environment a gate runs in: this process's, minus anything that would make it run itself. */
...
export function spawnArgv({ argv, cwd, timeoutMs, signal, env }) {
  return new Promise((resolve) => {
    ...
    const child = spawnChild(bin, list.slice(1), {
      ...
      // A gate must behave identically whether the runner was started from a shell or from inside
      // `node --test`. Node's test runner exports NODE_TEST_CONTEXT to its children, and a gate that
      // itself runs `node --test` INHERITS it — the grandchild then warns "run() is being called
      // recursively within a test file. skipping running files" and runs ZERO tests while exiting 0.
      // A vacuously green suite is the failure mode this repo has already paid for once, so the
      // variable is stripped rather than trusted.
      env: gateEnv(env),
...
/**
 * Where each internal handler comes from, one importer per id.
 *
 * A per-id map rather than one `Promise.all` over every module: `quick` now has handler gates of its
 * own (framework-tests.mjs), and importing the whole table would drag the release machinery —
 * package-clean, core-mounted, golden-replay — into every quick run, which is exactly what the
 * lazy import was for.
 */
const HANDLER_MODULES = Object.freeze({
  'tests.contract': async () => (await import('./gates/framework-tests.mjs')).testsContract,
  'tests.all': async () => (await import('./gates/framework-tests.mjs')).testsAll,
  parity: async () => (await import('./gates/framework-tests.mjs')).parity,
  'package.clean': async () => (await import('./gates/package-clean.mjs')).packageClean,
  'core.mounted': async () => (await import('./gates/core-mounted.mjs')).coreMounted,
  'golden.replay': async () => (await import('./gates/golden-replay.mjs')).goldenReplay,
  'installer.pwsh': async () => (await import('./gates/installer-pwsh.mjs')).installerPwsh,
});

/** The real internal-handler table for the SELECTED gates only. */
async function defaultHandlers(selected) {
  const wanted = [...new Set(selected.map((g) => g.handler).filter(Boolean))];
  const loaded = await Promise.all(wanted.map((id) => {
    const importer = HANDLER_MODULES[id];
    if (!importer) throw new SidekicksError(`check run: no handler registered for '${id}'`, EXIT_VALIDATION);
    return importer();
  }));
  return Object.fromEntries(wanted.map((id, i) => [id, loaded[i]]));
}
...
export async function runProfile({ repoRoot, profile, jobs = null, deps = {} } = {}) {
  ...
  const handlers = deps.handlers || (selected.some((g) => g.handler) ? await defaultHandlers(selected) : {});
    ...
    for (;;) {
      ...
      const ready = selected
        ...
        .filter((row) => row.status === 'pending'
          && row.gate.dependencies.every((d) => CLEARS.includes(rows.get(d)?.status)))
  ...
  // gates, so today every non-pass fails. `not_applicable` is the one exception, and it is not a
  // softening of that rule: it is the gate reporting that this environment has nothing for it to
  // judge, which is a different claim from "I did not judge it". See GATE_STATUS.
  const failed = gateRows.some((r) => r.blocking && !CLEARS.includes(r.status));
  const anyNotPassed = gateRows.some((r) => !CLEARS.includes(r.status));
...
async function executeGate({ row, repoRoot, spawn, handlers, timeoutMs, clock, signal }) {
  ...
  try {
    ...
    if (out.status && out.status !== 'passed' && out.status !== 'failed') {
      // A handler may report `skipped` — golden.replay does when the source repo has lost its
      // fixtures — or `not_applicable`, which it reports in a mount, where the fixtures are not
      // absent by accident but by design. See GATE_STATUS.
      row.status = out.status;
    } else {
      row.status = out.exitCode === 0 && !out.signal ? 'passed' : 'failed';
    }
    row.reason = out.reason
      ?? (CLEARS.includes(row.status)
        ? null
        : out.timedOut
