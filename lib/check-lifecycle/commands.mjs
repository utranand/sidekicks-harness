...
const MARK = Object.freeze({
  passed: 'PASS', failed: 'FAIL', blocked: 'BLOCK', skipped: 'SKIP', not_applicable: 'N/A',
});
...
export function renderHuman(result, jobs = null, deps = {}) {
  ...
  const counts = { passed: 0, failed: 0, blocked: 0, skipped: 0, not_applicable: 0 };
  ...
  lines.push(`${result.status === 'passed' ? 'PASSED' : 'FAILED'}`
    ...
    + (counts.not_applicable > 0 ? `, ${counts.not_applicable} n/a` : '')
  ...
  if (counts.not_applicable > 0) {
    lines.push('  an N/A gate has no subject in this environment — that is a fact about the '
      + 'environment, not a gap, and is distinct from a skip');
  }
  return `${lines.join('\n')}\n`;
}
