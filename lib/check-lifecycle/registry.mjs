...
export const GATES = Object.freeze([
  ...
  {
    id: 'tests.contract',
    // A handler, not an argv. A command gate is spawned with cwd = the WORKSPACE, and in a mounted
    // workspace these suites live at .sidekicks-core/lib/… — so a repo-relative argv resolved to
    // nothing and the gate failed in every consumer install (INC-2026-09-04-01, F-3). The handler
    // resolves them against the FRAMEWORK root instead. See gates/framework-tests.mjs.
    command: null,
    handler: 'tests.contract',
    dependencies: Object.freeze([]),
    blocking: true,
    cost_ms: 30_000,
  ...
  {
    id: 'tests.all',
    // Same reason as tests.contract: `scripts/` is at .sidekicks-core/scripts/ in a mount, so a
    // repo-relative argv found no launcher there.
    command: null,
    handler: 'tests.all',
  ...
  {
    id: 'parity',
    // Same reason again. These two suites are under tests/, which does NOT travel into a core, so
    // in a mount the handler fails with that named — rather than with a path that resolved nowhere.
    command: null,
    handler: 'parity',
    dependencies: Object.freeze(['tests.all']),
    blocking: true,
  ...
  },
  {
    ...
    blocking: true,
    // Raised from 240s for the added `package create` smoke, which is what proves `package.clean`
    // can resolve the framework from inside a mount (INC-2026-09-04-02, N-3).
    cost_ms: 300_000,
  ...
  },
  {
    id: 'installer.pwsh',
    command: null,
    handler: 'installer.pwsh',
    // No dependencies: it reads one template file and, where pwsh exists, parses it. Nothing it
    // checks is downstream of the suite, the catalog or an assembled package.
    dependencies: Object.freeze([]),
    // NOT blocking. On a host without pwsh the gate is `skipped` by design, and a blocking gate that
    // is skipped fails the profile — which would make `full` red on the primary dev machine (macOS,
    // no PowerShell) for a check that correctly reported it could not run. The structural lint
    // inside it still FAILS the gate when the template is malformed, on every host.
    blocking: false,
    cost_ms: 5_000,
    summary: 'Structurally check install.ps1.tmpl, and parse it wherever pwsh exists',
  },
]);
