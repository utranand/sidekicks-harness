...
test('the static registry is structurally valid and matches the specified gate table', () => {
  ...
  assert.deepEqual(GATES.map((g) => g.id), [
    ...
    'package.clean', 'core.mounted', 'golden.replay', 'installer.pwsh',
  ]);
  ...
  assert.deepEqual(deps, {
    ...
    // Depends on nothing: it reads one template file and, where pwsh exists, parses it. Making it
    // wait on the suite would only delay a 5-second check behind a two-minute one.
    'installer.pwsh': [],
  });
  ...
  for (const g of GATES) {
    ...
    // `installer.pwsh` is the one deliberate non-blocking gate: on a host with no pwsh it reports
    // `skipped`, and a blocking gate that is skipped fails the profile — which would make `full`
    // red on the primary dev machine (macOS) for a check that correctly said it could not run.
    // Its structural half still fails the gate on a malformed template, on every host.
    assert.equal(g.blocking, g.id !== 'installer.pwsh',
      `${g.id}: blocking is the default; only installer.pwsh opts out, and it documents why`);
  }
  // The test-running gates are handlers, not argv rows, and that is load-bearing: a command gate is
  // spawned with cwd = the WORKSPACE, so a repo-relative path to a framework FILE resolves to
  // nothing in a mounted workspace. See gates/framework-tests.mjs (INC-2026-09-04-01, F-3).
...
test('the three profiles compose as specified, each a superset of the last', () => {
  ...
  assert.deepEqual([...PROFILES.full], [...PROFILES.quick,
    'tests.all', 'framework.sync', 'config.sync', 'skill.doctor', 'parity', 'installer.pwsh']);
...
test('the PARITY suites resolve under the mount too, and travel inside lib/', async () => {
  // INC-2026-09-04-02, N-3. The contract suites got this treatment and the parity suites did not:
  // PARITY_SUITES still named repo-root `tests/`, which travels into neither a forged core nor a
  // package. So `check run full` was RED in every consumer install while green here, and the gate's
  // own failure message told the consumer their framework was incomplete.
  const { parity, PARITY_SUITES } = await import('../gates/framework-tests.mjs');

  for (const rel of PARITY_SUITES) {
    assert.ok(rel.startsWith(join('lib', '')),
      `${rel} must live under lib/ — that is the only tree copied whole into a core`);
  }

  const ws = mkdtempSync(join(tmpdir(), 'sk-gate-parity-'));
  try {
    const core = join(ws, '.sidekicks-core');
    mkdirSync(core, { recursive: true });
    writeFileSync(join(core, '.sidekicks-core.json'), '{"schema":1,"version":"1.0.0"}\n');
    ...
    let seen = null;
    const spawn = async (opts) => { seen = opts; return { exitCode: 0, signal: null, stdout: '', stderr: '' }; };
    const out = await parity({ repoRoot: ws, spawn, timeoutMs: 1000, signal: null });
    assert.equal(out.exitCode, 0);
    for (const rel of PARITY_SUITES) {
      assert.ok(seen.argv.includes(join(core, rel)),
        `the argv must name ${rel} inside the mount`);
    }
    assert.equal(seen.cwd, ws);
  } finally {
    rmSync(ws, { recursive: true, force: true });
  }
});

// not_applicable — a gate with no subject is not a gate that was skipped
// ---------------------------------------------------------------------------

test('a not_applicable blocking gate clears the profile, and a skipped one still fails it', async () => {
  // The distinction is the whole point. `golden.replay` in a mount has no fixtures and never will —
  // they are the source repo's contract snapshots and are deliberately not part of the forged
  // surface — so grading it `skipped` kept `check run release` permanently red in every consumer
  // install for a gate that had correctly concluded it had nothing to do (INC-2026-09-04-02, N-3).
  const gates = [handlerGate('subject', 'h.none'), handlerGate('after', 'h.ok', ['subject'])];

  const run = async (statusOfSubject) => runProfile({
    repoRoot,
    profile: 'one',
    deps: {
      gates,
      profiles: { one: ['subject', 'after'] },
      spawn: fakeSpawn(),
      clock: fakeClock(),
      onSignal: fakeSignals().register,
      handlers: {
        'h.none': async () => ({
          exitCode: null, signal: null, stdout: '', stderr: '',
          status: statusOfSubject, reason: 'nothing here to judge',
        }),
        'h.ok': async () => ({ exitCode: 0, signal: null, stdout: '', stderr: '' }),
      },
    },
  });

  const na = await run('not_applicable');
  assert.equal(byId(na.result, 'subject').status, 'not_applicable');
  assert.equal(byId(na.result, 'after').status, 'passed', 'a dependent must still run — nothing failed');
  assert.equal(na.result.status, 'passed');
  assert.equal(na.exitCode, 0, 'no subject is not a gap; it may not fail the profile');
  assert.match(renderHuman(na.result), /N\/A/);
  assert.match(renderHuman(na.result), /1 n\/a/);

  const sk = await run('skipped');
  assert.equal(byId(sk.result, 'subject').status, 'skipped');
  assert.equal(byId(sk.result, 'after').status, 'blocked', 'a skip is a non-pass and still blocks');
  assert.equal(sk.result.status, 'failed');
  assert.equal(sk.exitCode, 1, 'a skip is never reported as success');
});

test('golden.replay reports not_applicable in a mount and skipped in a bare repo', async () => {
  const { goldenReplay } = await import('../gates/golden-replay.mjs');

  const bare = mkdtempSync(join(tmpdir(), 'sk-golden-bare-'));
  const mount = mkdtempSync(join(tmpdir(), 'sk-golden-mount-'));
  try {
    const plain = await goldenReplay({ repoRoot: bare, spawn: async () => {
      throw new Error('must not spawn when there is nothing to replay');
    }, timeoutMs: 1000, signal: null });
    assert.equal(plain.status, 'skipped', 'a source repo that lost its fixtures has a real gap');

    mkdirSync(join(mount, '.sidekicks-core'), { recursive: true });
    writeFileSync(join(mount, '.sidekicks-core', '.sidekicks-core.json'), '{"schema":1,"version":"1.0.0"}\n');
    const mounted = await goldenReplay({ repoRoot: mount, spawn: async () => {
      throw new Error('must not spawn when there is nothing to replay');
    }, timeoutMs: 1000, signal: null });
    assert.equal(mounted.status, 'not_applicable');
    assert.match(mounted.reason, /never will be/);
    assert.match(mounted.reason, /check with no subject/);
  } finally {
